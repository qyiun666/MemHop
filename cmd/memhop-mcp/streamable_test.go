...
// TestStreamableMultiTenant boots the Streamable HTTP transport in-process and
// verifies per-request tenant routing: initialize and tools/list hit the
// tenant registry through the same /mcp/<tenant-id> path as SSE.
func TestStreamableMultiTenant(t *testing.T) {
	...
	base := memhop.MemHopConfig{
		...
		LLM: memhop.LlmConfig{
			APIURL: "http://localhost:9999/v1", APIKey: "smoke-key",
			Model: "smoke-model", TimeoutSecs: 30, MaxOutputTokens: 2048,
		},
	}
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	reg := newRegistry(base, dbDir, nil, logger)
