...
func registerL4Tools(s *mcp.Server, db *memhop.Session) {
	...
	description := fmt.Sprintf("检索 L4 内容（一轮的对话原文与其操作事件同住此层）：keyword（子串，忽略大小写）/ 时间范围 [start,end]（毫秒）/ ID 列表 / topic_id / kind / node_seq / content_type 都是过滤条件，填了的全部按 AND 组合；一个都不填即全域扫描。node_seq 是轮次内的步骤序号（由 Go 侧计划写面发号），取该步及其全部子步归因的记录，必须与 topic_id 同填。顺序看查询宽度：填了 topic_id 按该轮内 Seq 升序，不带 topic_id 的跨轮读取按记录自己的时间升序、同值以记录 id 收尾；limit 保留该顺序末尾的 N 条（缺省 %d，可填更大值）。text/document/code 存原文，image/audio/video 等媒体类型的 content 为路径。返回记录里的三个数值字段自有词表：kind 0=utterance / 1=event；role 0=user / 1=agent / 2=system，而 role 3 是库盖在融合组摘要上的自有标记——写侧不接受它，它也没有宿主起的名字；type 0=text / 1=image / 2=video / 3=document / 4=audio / 5=code / 255=other，其中 event 记录恒为 0（事件没有媒介）。", archiveSearchDefaultLimit)
	...
	s.AddTool(&mcp.Tool{
		...
		InputSchema: objSchema(map[string]any{
			...
			"topic_id":     strProp("限定话题 ID（16 位 hex）；不填或空串即不限定，跨话题按记录时间扫全域"),
		...
		// somebody said; a search that says no kind wants both kinds. The topic key
		// follows the same rule: an empty one is not an address, and a client that
		// sends "" means "no topic condition".
