//! MemHop v0.18.0 MCP Server — 5层记忆架构的 JSON-RPC 2.0 接口。
...
fn get_or_open_brain(agent_id: &str, brains_dir: &str, model_path: Option<String>) -> Result<Arc<Mutex<Brain>>, String> {
...
async fn handle(stream: UnixStream, brains_dir: String, model_path: Option<String>) {
    ...
    loop {
        ...
        let response = match method {
            "memhop_batch_store"    => handle_batch_store(&id, agent_id, &brains_dir, &model_path, &params),
            ...
            "memhop_mount_shelf"    => handle_mount_shelf(&id, agent_id, &brains_dir, &model_path, &params),
            "memhop_unmount_shelf"  => handle_unmount_shelf(&id, agent_id, &brains_dir, &model_path, &params),
            ...
            "memhop_set_profile"     => handle_set_profile(&id, agent_id, &brains_dir, &model_path, &params),
            "memhop_get_activated"  => handle_get_activated(&id, agent_id, &brains_dir, &model_path, &params),
            "memhop_activate"       => handle_activate(&id, agent_id, &brains_dir, &model_path, &params),
            "memhop_deactivate"     => handle_deactivate(&id, agent_id, &brains_dir, &model_path, &params),
            "memhop_get_l4_raw"     => handle_get_l4_raw(&id, agent_id, &brains_dir, &model_path, &params),
            ...
            "memhop_re_search"      => handle_re_search(&id, agent_id, &brains_dir, &model_path, &params),
            "memhop_list_shelf"     => handle_list_shelf(&id, agent_id, &brains_dir, &model_path),
            "memhop_update_topic"  => handle_update_topic(&id, agent_id, &brains_dir, &model_path, &params),
...
fn handle_batch_store(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
        ...
        Some(arr) => {
            ...
            for item in arr {
                ...
                result.push(StoreItem {
                    ...
                    importance: item.get("importance").and_then(|v| v.as_f64()).map(|v| v as f32),
                });
            }
            result
        }
...
fn handle_recall(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
    ...
    let spread_depth = params.get("spread_depth").and_then(|v| v.as_u64()).map(|v| v as usize);
    let topic_filter = params.get("topic_filter").and_then(|v| v.as_str()).map(|s| s.to_string());

    let exclude_ids = params.get("exclude_ids").and_then(|v| v.as_array()).map(|arr| {
        arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
    }).unwrap_or_default();

    let exclude_topic_ids = params.get("exclude_topic_ids").and_then(|v| v.as_array()).map(|arr| {
        arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
    }).unwrap_or_default();

    let l3_domain_id = params.get("l3_domain_id").and_then(|v| v.as_str()).map(|s| s.to_string());
    let l2_topic_id = params.get("l2_topic_id").and_then(|v| v.as_str()).map(|s| s.to_string());
    ...
    let req = RecallRequest {
        ...
        l2_topic_id,
        session_id,
        time_decay_lambda: params.get("time_decay_lambda").and_then(|v| v.as_f64()).map(|v| v as f32),
    };
...
fn handle_dream(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_organize(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_mount_shelf(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
    ...
    let doc_type_str = params.get("doc_type").and_then(|v| v.as_str()).unwrap_or("generic");
...
fn handle_unmount_shelf(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_get_profile(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_set_profile(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
    let role_name = params.get("role_name").and_then(|v| v.as_str()).map(|s| s.to_string());
    let role = params.get("role").and_then(|v| v.as_str()).map(|s| s.to_string());
    let position = params.get("position").and_then(|v| v.as_str()).map(|s| s.to_string());
    let traits: HashMap<String, String> = params.get("traits").and_then(|v| v.as_object()).map(|obj| {
    ...
    match guard.set_l0_profile(role_name, role, position, traits) {
        Ok(()) => json!({"jsonrpc":"2.0","id":id,"result":{"status":"ok"}}),
        Err(e) => error_response(id, -32000, &e.to_string()),
    }
}

fn handle_get_activated(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, _params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_activate(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
    ...
    let ttl_ms = params.get("ttl_ms").and_then(|v| v.as_i64()).unwrap_or(3_600_000);
...
fn handle_deactivate(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_get_l4_raw(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_list_l3_paths(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_list_topics(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_re_search(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
    ...
    let target_layers = params.get("target_layers").and_then(|v| v.as_array()).map(|arr| {
        arr.iter().filter_map(|v| v.as_str()).filter_map(|s| match s {
            "L1" => Some(Layer::L1), "L2" => Some(Layer::L2),
            "L3" => Some(Layer::L3), "L4" => Some(Layer::L4), _ => None,
        }).collect()
    }).unwrap_or_else(|| vec![Layer::L1, Layer::L2, Layer::L4]);
    let exclude_ids = params.get("exclude_ids").and_then(|v| v.as_array()).map(|arr| {
        arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
    ...
    let req = RecallRequest {
        query, max_results, target_layers, time_range: None,
        spread_depth: params.get("spread_depth").and_then(|v| v.as_u64()).map(|v| v as usize),
        topic_filter: params.get("topic_filter").and_then(|v| v.as_str()).map(|s| s.to_string()),
        exclude_ids, exclude_topic_ids,
        l3_domain_id: params.get("l3_domain_id").and_then(|v| v.as_str()).map(|s| s.to_string()),
        l2_topic_id: params.get("l2_topic_id").and_then(|v| v.as_str()).map(|s| s.to_string()),
        session_id: params.get("session_id").and_then(|v| v.as_str()).map(|s| s.to_string()),
        time_decay_lambda: params.get("time_decay_lambda").and_then(|v| v.as_f64()).map(|v| v as f32),
    };
    let mut guard = match brain.lock() {
        Ok(g) => g,
...
fn handle_list_shelf(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_feedback(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };

    let result_ids: Vec<String> = params.get("result_ids")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
        .unwrap_or_default();

    let relevant = params.get("relevant").and_then(|v| v.as_bool()).unwrap_or(true);
    let session_id = params.get("session_id").and_then(|v| v.as_str()).unwrap_or("default");
...
fn handle_stats(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
...
fn handle_update_topic(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
    ...
    let extended_meta = params.get("extended_meta").and_then(|v| v.as_object()).map(|obj| {
        obj.iter().map(|(k, v)| {
            let val = match v {
...
fn handle_set_l0(id: &Value, agent_id: &str, brains_dir: &str, model_path: &Option<String>, params: &serde_json::Map<String, Value>) -> Value {
    let brain = match get_or_open_brain(agent_id, brains_dir, model_path.clone()) {
        Ok(b) => b,
        Err(e) => return error_response(id, -32000, &e),
    };
    ...
    let role_name = params.get("role_name").and_then(|v| v.as_str()).map(|s| s.to_string());
    let personality = params.get("personality").and_then(|v| v.as_array()).map(|a| {
        a.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
    }).unwrap_or_default();
    let values = params.get("values").and_then(|v| v.as_array()).map(|a| {
        a.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
    }).unwrap_or_default();
    let worldview = params.get("worldview").and_then(|v| v.as_array()).map(|a| {
        a.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
    }).unwrap_or_default();
    let traits: std::collections::HashMap<String, String> = params.get("traits").and_then(|v| v.as_object()).map(|obj| {
        obj.iter().filter_map(|(k, v)| v.as_str().map(|s| (k.clone(), s.to_string()))).collect()
    }).unwrap_or_default();
    ...
    match guard.set_l0(role_name, personality, values, worldview, traits) {
        Ok(()) => json!({"jsonrpc":"2.0","id":id,"result":{"status":"ok"}}),
        Err(e) => error_response(id, -32000, &e.to_string()),
    }
}
