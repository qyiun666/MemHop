...
107. **CI 与本地验证口径的三处错配修一处、坐实一处、认下一处**（`.github/workflows/workflow.yml`、`test/fidelity_test.go`；无库行为改动）。① **CI 装的 Go 比模块要求的低**：`go.mod` 自 `9549159` 起要求 `go 1.27.0`，workflow 仍钉 `go-version: '1.26'`——本机实测同构错误为 `go: go.mod requires go >= 1.99.0 (running go 1.27.0; GOTOOLCHAIN=local)`，也就是说这一步要么硬失败、要么靠隐式下载另一个工具链把钉过的版本变成装饰品。改成 `go-version-file: go.mod`：版本的唯一出处回到模块本身，CI 不再持有第二份事实（这正是本仓一路在做的「一处判断一处实现」，只是这次漏在 CI 配置里）。② **`make check-guides` 此前只在本地钩子里**：两份指南的可复制骨架是「照文档抄就能集成」这句话的机器检查，但 CI 从不跑它——补上（非 Windows 步骤，本机 `guide skeletons build (2 guides)` 验过）。顺带把只覆盖 Linux 的 integration 编译步骤放宽到非 Windows：darwin 上 `go vet -tags integration ./test/...` 与 `go test -tags=integration` 的编译路径本机已跑通；CI 上没有凭据文件时 `testsupport` 走 `errNoLLMConfig` 跳过，这条已由读码确认（env 优先、其次 `test/testsupport/key_config.json`，两者都写在 `.gitignore` 里，仓库不带凭据）。③ **`TestDreamCompressionFidelity` 那次红是判官的错，不是引擎的**：它把 25 条原文喂进一个场景，却只把 `related[:4]` 拿给评审模型看，注释还写着「核心 4 条装着全部事实」——于是从第 5 到第 25 条里正经蒸出来的关键词（播客、蛋白粉、最舒服……）被逐条判成幻觉。改成把全部 25 条交给判官，判据本身不动。**这条改完我还没付一次真额度跑去验**——本地那次红正是我为了复现 CI 步骤而误花的：`go test -tags=integration` 在有本地凭据的机器上会真调模型，往后本地只做 `go vet -tags integration`，跑不调先问。
