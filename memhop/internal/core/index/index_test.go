...
func TestIVFIndex(t *testing.T) {
	t.Run("add_and_search", func(t *testing.T) {
		ivf := NewIVFIndex(4, 2)
		v1 := []uint16{F32ToF16(1.0), F32ToF16(0.0), F32ToF16(0.0), F32ToF16(0.0)}
		v2 := []uint16{F32ToF16(0.0), F32ToF16(1.0), F32ToF16(0.0), F32ToF16(0.0)}
		v3 := []uint16{F32ToF16(0.9), F32ToF16(0.1), F32ToF16(0.0), F32ToF16(0.0)}

		ivf.AddVector(1, v1, 10, 0)
		ivf.AddVector(2, v2, 11, 0)
		ivf.AddVector(3, v3, 12, 0)

		if ivf.K != 2 {
			t.Errorf("K should be 2, got %d", ivf.K)
		}
		if ivf.Len() != 3 {
			t.Errorf("total vectors should be 3, got %d", ivf.Len())
		}
	})

	t.Run("rebuild", func(t *testing.T) {
		ivf := NewIVFIndex(4, 2)
		v := []uint16{F32ToF16(1.0), F32ToF16(0.0), F32ToF16(0.0), F32ToF16(0.0)}
		for id := uint64(1); id <= 16; id++ {
			ivf.AddVector(id, v, uint32(id), 0)
		}
		ivf.RebuildIfNeeded(ivf.Len())
		// sqrt(16) = 4, initial_k = 2 → target 4
		if ivf.K != 4 {
			t.Errorf("K should be 4 after rebuild, got %d", ivf.K)
		}
		if ivf.Len() != 16 {
			t.Errorf("total vectors should still be 16, got %d", ivf.Len())
		}
	})
}

func TestIVFSerializationRoundtrip(t *testing.T) {
	dim := 8
	ivf := NewIVFIndex(dim, 2)
	v1 := makeF16Vec(dim, 1.0)
	v2 := makeF16Vec(dim, 0.0)
	v3 := makeF16Vec(dim, 0.5)
	ivf.AddVector(101, v1, 7, 0)
	ivf.AddVector(102, v2, 8, 1)
	ivf.AddVector(103, v3, 9, 0)

	dir := t.TempDir()
	path := filepath.Join(dir, "test.meh")
	engine, err := storage.Create(path, uint16(dim))
	if err != nil {
		t.Fatal(err)
	}

	if err := WriteIVFIndex(engine, ivf); err != nil {
		t.Fatal(err)
	}

	restored, err := ReadIVFIndex(engine)
	if err != nil {
		t.Fatal(err)
	}
	if restored == nil {
		t.Fatal("ReadIVFIndex returned nil")
	}
	if restored.Dim != ivf.Dim {
		t.Errorf("dim mismatch: %d vs %d", restored.Dim, ivf.Dim)
	}
	if restored.K != ivf.K {
		t.Errorf("K mismatch: %d vs %d", restored.K, ivf.K)
	}
	if restored.Len() != ivf.Len() {
		t.Errorf("len mismatch: %d vs %d", restored.Len(), ivf.Len())
	}

	engine.Close(&storage.IndexSnapshotData{})
}

func TestIVFKNN(t *testing.T) {
	dim := 4
	ivf := NewIVFIndex(dim, 2)

	// Create vectors and store them in the engine too.
	dir := t.TempDir()
	path := filepath.Join(dir, "knn.meh")
	engine, err := storage.Create(path, uint16(dim))
	if err != nil {
		t.Fatal(err)
	}
	...
	vectors := map[uint64][]uint16{
		1: {F32ToF16(1.0), F32ToF16(0.0), F32ToF16(0.0), F32ToF16(0.0)},
		2: {F32ToF16(0.0), F32ToF16(1.0), F32ToF16(0.0), F32ToF16(0.0)},
		3: {F32ToF16(0.9), F32ToF16(0.1), F32ToF16(0.0), F32ToF16(0.0)},
	}

	for id, vec := range vectors {
		vecHash := VecRecordHash(id)
		data := encodeF16Vec(vec)
		engine.WriteRecord(storage.RecIVFCluster, vecHash, data)
		ivf.AddVector(id, vec, 0, 0)
	}

	query := []uint16{F32ToF16(0.95), F32ToF16(0.05), F32ToF16(0.0), F32ToF16(0.0)}
	results, err := IVFKNN(ivf, engine, query, 3, 2)
	if err != nil {
		t.Fatal(err)
	}
	if len(results) == 0 {
		t.Error("IVFKNN should return results")
	}
	// The most similar should be vector 1 (closest to [0.95, 0.05, 0, 0])
	if len(results) > 0 && results[0].IDHash != 1 {
		t.Errorf("closest vector should be id=1, got %d", results[0].IDHash)
	}
}
