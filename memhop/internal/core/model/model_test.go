...
func TestProfileSlotRoundtrip(t *testing.T) {
	...
	if got.IDHash != p.IDHash || got.Name != p.Name || got.Version != p.Version {
...
// ============================================================================
// L1 ContextNode + SceneNode
// ============================================================================

func TestContextNodeRoundtrip(t *testing.T) {
	n := ContextNode{
		IDHash: 12345, ContextID: 67890, VectorPageRef: 42,
		Importance: 0.85, Valence: 0.0, Arousal: 0.0,
		CreatedAt: 1000000, UpdatedAt: 2000000, Version: 1,
		EdgePtrs: []uint64{100, 200, 300},
	}
	var got ContextNode
	jsonRoundtrip(t, n, &got)
	if got.IDHash != n.IDHash || got.ContextID != n.ContextID {
		t.Fatalf("mismatch: %+v", got)
	}
	if len(got.EdgePtrs) != 3 {
		t.Fatalf("edge_ptrs length mismatch")
	}
}

func TestContextNodeEmptyEdges(t *testing.T) {
	n := ContextNode{EdgePtrs: []uint64{}}
	var got ContextNode
	jsonRoundtrip(t, n, &got)
	if len(got.EdgePtrs) != 0 {
		t.Fatalf("expected empty edge_ptrs")
	}
}
...
func TestSceneNodeRoundtrip(t *testing.T) {
	n := SceneNode{
		...
		Depth: 2, VectorPageRef: 50, Importance: 0.9,
	...
	var got SceneNode
	jsonRoundtrip(t, n, &got)
	if got.SceneID != n.SceneID || got.Depth != n.Depth {
		t.Fatalf("mismatch: %+v", got)
	}
...
// ============================================================================
// L1 HyperedgeSlot + SceneEdge
// ============================================================================

func TestHyperedgeSlotRoundtrip(t *testing.T) {
	s := HyperedgeSlot{
		IDHash: 1234567890, Kind: HyperSemantic,
		NodePtrs: []uint64{100, 200, 300}, Weight: 0.85,
		CreatedAt: 1234567890, UpdatedAt: 1234567891,
		Version: 1, OverflowPage: 0,
	}
	var got HyperedgeSlot
	jsonRoundtrip(t, s, &got)
	if got.Kind != HyperSemantic || got.Weight != 0.85 {
		t.Fatalf("mismatch: %+v", got)
	}
}

func TestHyperedgeAllKinds(t *testing.T) {
	kinds := []HyperedgeKind{
		HyperCoOccurrence, HyperCausal, HyperSemantic,
		HyperTemporal, HyperHierarchical, HyperSequence,
	}
	for _, k := range kinds {
		s := HyperedgeSlot{IDHash: 1, Kind: k, NodePtrs: []uint64{1}}
		var got HyperedgeSlot
		jsonRoundtrip(t, s, &got)
		if got.Kind != k {
...
func TestTopicSlotRoundtripDepth1(t *testing.T) {
	...
	if got.FusedSummary != nil {
		t.Fatalf("depth-1 should have nil fused_summary")
	}
}
...
func TestTopicSlotRoundtripDepth2(t *testing.T) {
	...
	if got.FusedSummary == nil || *got.FusedSummary != "认证流程讨论" {
		t.Fatalf("fused_summary mismatch")
	}
}
...
func TestTopicSlotUnicode(t *testing.T) {
	...
	summary := "摘要 📝"
	topic.FusedSummary = &summary
	var got TopicSlot
	jsonRoundtrip(t, topic, &got)
	if got.UserKeywords[0] != "场景 🚀" {
		t.Fatalf("unicode keyword mismatch")
	}
	if *got.FusedSummary != "摘要 📝" {
		t.Fatalf("unicode summary mismatch")
	}
}
...
func TestHypergraphSourceJSONFormats(t *testing.T) {
	tests := []struct {
		...
		{HypergraphSource{Kind: SourcePath, Value: "/a/b"}, `{"Path":"/a/b"}`},
		{HypergraphSource{Kind: SourceContext, ContextID: 42}, `{"Context":42}`},
		{HypergraphSource{Kind: SourceURL, Value: "http://x"}, `{"Url":"http://x"}`},
		{HypergraphSource{Kind: SourceManual}, `"Manual"`},
	}
...
func TestHypergraphNodeHexJSON(t *testing.T) {
	...
	var idHex string
	if err := json.Unmarshal(raw["id_hash"], &idHex); err != nil {
		t.Fatalf("unmarshal id_hash: %v", err)
	}
	if idHex != "00000000deadbeef" {
		t.Fatalf("hex mismatch: %s", idHex)
	}
}
...
func TestHypergraphEdgeRoundtrip(t *testing.T) {
	e := HypergraphEdge{
		...
		Label: strPtr("depends_on"), Confidence: 0.9,
		Description: nil, ValidFrom: 0, ValidUntil: 0, CreatedAt: 1000,
	}
...
func TestHypergraphEdgeAllKinds(t *testing.T) {
	...
	for _, k := range kinds {
		e := HypergraphEdge{
			...
			NodeIDs: []uint64{1, 2}, Weight: 0.5, Confidence: 0.9,
		}
		var got HypergraphEdge
		jsonRoundtrip(t, e, &got)
		if got.Kind != k {
...
func makeTopic(id uint64, depth uint8) TopicSlot {
	...
	return TopicSlot{
		...
		UserL4Refs: []uint64{10}, UserL3Refs: []uint64{20},
