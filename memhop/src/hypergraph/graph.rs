...
use crate::index::{SparseIndex, HnswIndex};
...
impl L1Hypergraph {
    pub fn new() -> Self {
        L1Hypergraph { bm25: SparseIndex::new(), vector_index: HnswIndex::default(), node_count: 0 }
    }

    /// v0.16.0: 使用指定维度创建。
    pub fn with_dim(dim: usize) -> Self {
        L1Hypergraph { bm25: SparseIndex::new(), vector_index: HnswIndex::new(dim), node_count: 0 }
    }

    /// v0.18.0: 使用指定维度和配置创建。
    pub fn with_dim_and_config(dim: usize, config: crate::index::HnswConfig) -> Self {
        L1Hypergraph { bm25: SparseIndex::new(), vector_index: HnswIndex::new_with_config(dim, config), node_count: 0 }
    }
    ...
    pub fn add_node_with_id(&mut self, wtxn: &mut RwTxn<'_>, env: &L1Env,
        ...
        let id = id.to_string();
        let mut node = KnowledgeNode::new(id.clone(), text.to_string(), sparse.clone(), vector.clone(), Layer::L1, source);
