...
pub(crate) fn execute(brain: &mut Brain, batch: StoreBatch) -> Result<BatchReport> {
    ...
    let mut l4_doc_ids: Vec<String> = Vec::new();
    {
        ...
        for item in &encoded {
            let doc_id = brain.l4.store_with_id(&mut wtxn, &brain.l4_env,
                &unique_id("l4d"), &item.text, &item.source,
        ...
        for item in &encoded {
            ...
            if !item.vector.is_empty() && !brain.l1.vector_index.is_empty() {
                ...
                for (cand_id, sim) in &candidates {
                    if *sim > 0.95 {
                        // Check ngram overlap
                        if let Ok(Some(cand_node)) = brain.l1.get_node(&wtxn, &brain.l1_env, cand_id) {
                            let intersection = item.sparse.keys().filter(|k| cand_node.sparse.contains_key(*k)).count() as f32;
                            ...
                            if jaccard > 0.8 {
                                ...
                                brain.l1_env.nodes.put(&mut wtxn, cand_id, &bytes).map_err(|e| MemHopError::Storage(e.to_string()))?;
            ...
            if !dedup_found {
                let node_id = brain.l1.add_node_with_id(&mut wtxn, &brain.l1_env,
                    &unique_id("kn"), &item.text, &item.sparse, item.vector.clone(),
                    item.llm_keywords.clone().unwrap_or_default(),
                    NodeSource::Perception, item.importance)?;
                    ...
                    && let Ok(Some(bytes)) = brain.l1_env.nodes.get(&wtxn, &node_ids[i])
                        && let Ok(mut node) = bincode::deserialize::<crate::engram::KnowledgeNode>(bytes) {
                            ...
                            if let Ok(new_bytes) = bincode::serialize(&node) {
                                brain.l1_env.nodes.put(&mut wtxn, &node_ids[i], &new_bytes)
    ...
    // Phase 4: L2 topic update — 带 llm_compressed_summary + 真实 node_id + doc_ids + linked_domain_ids + centroid
    {
        ...
        for (i, item) in encoded.iter().enumerate() {
            if let Some(ref label) = item.topic_label {
                ...
                // 填充 doc_ids (L4 文档 ID)
                if i < l4_doc_ids.len() && !l4_doc_ids[i].is_empty()
                ...
                if topic_changed {
                    ...
                    if let Some(ref domain_id) = item.domain_id {
                        let current_weight = topic.domain_weights.get(domain_id).copied().unwrap_or(0.0);
                        topic.domain_weights.insert(domain_id.clone(), current_weight + 1.0);
                    }
                    // 2. 节点关联强度：基于节点与话题的相关性（这里简化为1.0）
                    if i < node_ids.len() {
                        let node_id = &node_ids[i];
                        let current_weight = topic.node_weights.get(node_id).copied().unwrap_or(0.0);
                    ...
                    match topic.dialogue_range {
                        Some((start, end)) => {
                            if now_ms < start { topic.dialogue_range = Some((now_ms, end)); }
                            if now_ms > end { topic.dialogue_range = Some((start, now_ms)); }
                        }
                        None => { topic.dialogue_range = Some((now_ms, now_ms)); }
                    }
                ...
                if i < node_ids.len() && !item.vector.is_empty() {
                    topic_new_nodes.entry(topic_id.clone()).or_default()
        ...
        for (tid, nodes) in &topic_new_nodes {
            if let Err(e) = brain.l2.update_topic_centroid(&mut wtxn, &brain.l2_env, tid, nodes) {
    ...
    // Phase 5: L3 domain update — 自动生成 L3 领域
    {
        ...
        for item in &encoded {
            ...
            if !domain_cache.contains(&domain_id) {
                ...
                if brain.l3_env.domain_meta.get(&wtxn, &meta_key)
                    .map_err(|e| MemHopError::Storage(e.to_string()))?.is_none() 
                {
                    // 领域不存在，创建新领域
                    let name = if domain_id == "domain_default" {
                        "默认领域".to_string()
                    } else {
                        domain_id.strip_prefix("domain_").unwrap_or(&domain_id).to_string()
                    };
            ...
            brain.l3.add_node_with_id(&mut wtxn, &brain.l3_env, &l3_id, &domain_id, &item.text, &item.sparse, "", item.vector.clone())?;
            report.l3_nodes_created += 1;
            // v0.17.3: 记录输入项到 L3 节点 ID 的映射
            input_first_l3_node.entry(item.input_index).or_insert(l3_id.clone());
            eprintln!("[batch_store] L3 node created: l3_id={}, input_index={}", l3_id, item.input_index);
        }
        wtxn.commit().map_err(|e| MemHopError::Storage(e.to_string()))?;
    }
    ...
    // Debug: 打印映射信息
    eprintln!("[batch_store] engram_ids: {:?}, l3_engram_ids: {:?}", report.engram_ids, report.l3_engram_ids);
