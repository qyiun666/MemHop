...
pub fn get_plan_tree(brain: &Brain, session_id: &str) -> Result<Vec<crate::engram::Topic>> {
    ...
    let node_ids: Vec<String> = match brain.l4_env.session_index.get(&txn, &session_key)
        .map_err(|e| MemHopError::Storage(e.to_string()))?
    {
...
pub fn complete_plan(brain: &mut Brain, topic_id: &str) -> Result<()> {
    ...
    if !topic.node_ids.is_empty() {
        brain.l1.add_hyperedge(&mut wtxn, &brain.l1_env,
            ...
            crate::types::HyperedgeKind::Merged, 1.0,
            None, Some("plan_complete".to_string()))?;
    }
...
pub fn consolidate_plan_summaries(brain: &mut Brain) -> Result<u32> {
    ...
    for topic in &topics {
        if topic.node_ids.is_empty() { continue; }
        ...
        for nid in &topic.node_ids {
            if let Ok(Some(node)) = brain.l1.get_node(&l1_txn, &brain.l1_env, nid) {
                ...
                let text = node.summary.unwrap_or_else(|| {
                    node.text.chars().take(100).collect()
                });
        ...
        if summary_parts.is_empty() { continue; }
