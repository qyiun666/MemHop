...
pub fn associative_recall(brain: &Brain, req: &RecallRequest) -> Result<RecallResponse> {
    ...
    let l1_results = query_engine::search_l1(brain, &encoded.sparse, &encoded.dense, req.max_results)?;
    ...
    let seed_ids: Vec<String> = l1_results.iter()
        .take(3)
        .map(|r| r.id.clone())
        .collect();
...
fn merge_rrf(primary: Vec<RecallResult>, secondary: Vec<RecallResult>, max: usize) -> Vec<RecallResult> {
    let k = 60.0f64;
...
mod tests {
    ...
    fn test_merge_rrf_max_limit() {
        let primary: Vec<RecallResult> = (0..5).map(|i| RecallResult {
            layer: Layer::L1,
            id: format!("kn_{}", i),
            text: "test".to_string(),
            score: 1.0,
        ...
        let secondary: Vec<RecallResult> = (5..10).map(|i| RecallResult {
            layer: Layer::L1,
            id: format!("kn_{}", i),
            text: "test".to_string(),
            score: 0.5,
