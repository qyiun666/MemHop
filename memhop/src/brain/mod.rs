...
use crate::types::{BrainConfig, StoreBatch, BatchReport, RecallRequest, RecallResponse, ConsolidateReport, L0Profile, ActivatedTopicInfo, L3PathInfo};
...
use crate::engram::{Topic, RawDocument};
use crate::types::DreamConfig;

/// MemHop v0.17.0 Brain — 5 层仿人脑记忆架构顶层 API。
pub struct Brain {
...
impl Brain {
    pub fn open(config: BrainConfig) -> Result<Self> {
        ...
        let encoder: Box<dyn Encoder> = if let Some(ref _mp) = config.model_path {
            // v0.18.0: 暂时禁用candle feature，使用NgramEncoder
            eprintln!("memhop-brain: using NgramEncoder (candle feature disabled)");
        ...
        Ok(Brain { config, l0_env, l1_env, l2_env, l3_env, l4_env, l0, l1, l2, l3, l4, session_mgr, encoder })
    }
    ...
    pub fn recall(&mut self, req: &RecallRequest) -> Result<RecallResponse> {
        ...
        // 附带 L0 Profile
        let txn = self.l0_env.env.read_txn()
    ...
    /// 设置 L0 角色画像（身份字段：role_name, role, position, traits）
    pub fn set_l0_profile(&mut self, role_name: Option<String>, role: Option<String>,
        position: Option<String>, traits: std::collections::HashMap<String, String>) -> Result<()> {
        let env = self.l0_env.env.clone();
        let mut wtxn = env.write_txn()
            .map_err(|e| MemHopError::Storage(e.to_string()))?;
        // 读取现有 profile 或创建新的
        let mut profile = self.l0.get_profile(&wtxn, &self.l0_env)?
            .unwrap_or_default();
        if let Some(name) = role_name { profile.role_name = Some(name); }
        if let Some(r) = role { profile.role = Some(r); }
        if let Some(p) = position { profile.position = Some(p); }
        if !traits.is_empty() { profile.traits.extend(traits); }
        profile.version += 1;
    ...
    pub fn set_l0(&mut self, role_name: Option<String>, personality: Vec<String>,
        values: Vec<String>, worldview: Vec<String>,
        traits: std::collections::HashMap<String, String>) -> Result<()> {
        let env = self.l0_env.env.clone();
        let mut wtxn = env.write_txn()
            .map_err(|e| MemHopError::Storage(e.to_string()))?;
        let mut profile = self.l0.get_profile(&wtxn, &self.l0_env)?
            .unwrap_or_default();
        if let Some(name) = role_name { profile.role_name = Some(name); }
        if !personality.is_empty() { profile.personality = personality; }
        if !values.is_empty() { profile.values = values; }
        if !worldview.is_empty() { profile.worldview = worldview; }
        if !traits.is_empty() { profile.traits = traits; }
        profile.updated_at = chrono::Utc::now().timestamp_millis();
        profile.version += 1;
        self.l0.update_profile(&mut wtxn, &self.l0_env, &profile)?;
        wtxn.commit().map_err(|e| MemHopError::Storage(e.to_string()))?;
    ...
    pub fn update_topic(&mut self, topic_id: &str,
        summary: Option<String>, keywords: Option<Vec<String>>,
        extended_meta: Option<std::collections::HashMap<String, String>>) -> Result<()> {
        let env = self.l2_env.env.clone();
        let mut wtxn = env.write_txn()
            .map_err(|e| MemHopError::Storage(e.to_string()))?;
        let key = format!("topic:{}:meta", topic_id);
        match self.l2_env.topics.get(&wtxn, &key)
            .map_err(|e| MemHopError::Storage(e.to_string()))?
        {
            Some(bytes) => {
                if let Ok(mut topic) = bincode::deserialize::<crate::engram::Topic>(bytes) {
                    if let Some(ref s) = summary { topic.summary = Some(s.clone()); }
                    if let Some(ref kw) = keywords { topic.keywords = kw.clone(); }
                    if let Some(ref meta) = extended_meta { topic.extended_meta = meta.clone(); }
                    topic.updated_at = chrono::Utc::now().timestamp_millis();
                    let new_bytes = bincode::serialize(&topic)
                        .map_err(|e| MemHopError::Storage(e.to_string()))?;
                    self.l2_env.topics.put(&mut wtxn, &key, &new_bytes)
                        .map_err(|e| MemHopError::Storage(e.to_string()))?;
                }
            }
    ...
    pub fn get_l4_raw(&self, doc_id: &str) -> Result<Option<RawDocument>> {
        let txn = self.l4_env.env.read_txn()
            .map_err(|e| MemHopError::Storage(e.to_string()))?;
        match self.l4_env.docs.get(&txn, doc_id).map_err(|e| MemHopError::Storage(e.to_string()))? {
    ...
    pub fn list_l3_paths(&self) -> Result<Vec<L3PathInfo>> {
        let txn = self.l3_env.env.read_txn()
            .map_err(|e| MemHopError::Storage(e.to_string()))?;
    ...
    pub fn list_topics(&self) -> Result<Vec<Topic>> {
        let txn = self.l2_env.env.read_txn()
            .map_err(|e| MemHopError::Storage(e.to_string()))?;
        ...
        if let Ok(iter) = self.l2_env.topics.iter(&txn) {
            for (key, bytes) in iter.flatten() {
                ...
                match bincode::deserialize::<Topic>(bytes) {
                    Ok(t) => topics.push(t),
                    Err(e) => eprintln!("[brain] list_topics: deserialize error for key '{}': {}", key, e),
                }
            }
        }
        Ok(topics)
