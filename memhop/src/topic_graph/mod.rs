...
impl L2TopicGraph {
    pub fn new() -> Self {
        L2TopicGraph { topic_vectors: HnswIndex::default() }
    }

    /// v0.16.0: 使用指定维度创建。
    pub fn with_dim(dim: usize) -> Self {
        L2TopicGraph { topic_vectors: HnswIndex::new(dim) }
    }

    /// v0.18.0: 使用指定维度和配置创建。
    pub fn with_dim_and_config(dim: usize, config: crate::index::HnswConfig) -> Self {
        L2TopicGraph { topic_vectors: HnswIndex::new_with_config(dim, config) }
    }

    /// 从 LMDB 重建 topic 向量索引。
    pub fn rebuild_topic_vectors(&mut self, env: &L2Env) -> Result<()> {
        let txn = env.env.read_txn()
        ...
        self.topic_vectors = if dim > 0 { HnswIndex::new(dim) } else { HnswIndex::default() };
    ...
    pub fn persist_topic_vectors(&self, wtxn: &mut RwTxn<'_>, env: &L2Env) -> Result<()> {
        ...
        env.topic_vector_index.put(wtxn, VECTOR_INDEX_KEY, &bytes)
            .map_err(|e| MemHopError::Storage(e.to_string()))
    }

    /// 计算并更新 topic 的 centroid（所有成员 node 向量均值）。
    pub fn update_topic_centroid(&mut self, wtxn: &mut RwTxn<'_>, env: &L2Env,
        topic_id: &str, l1_nodes: &[(String, Vec<f16>)]) -> Result<()> {
        if l1_nodes.is_empty() { return Ok(()); }
    ...
    pub fn get_topics_by_domain(&self, txn: &heed::RoTxn<'_>, env: &L2Env, domain_id: &str) -> Result<Vec<Topic>> {
