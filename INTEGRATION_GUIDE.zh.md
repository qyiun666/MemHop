...
- **把场景读折成进 prompt 的几行，是四个判定，这里一次说清。** `SceneContext("")` 交出的清单是
  **刻意摊平到 depth ≤ 2** 的：一个巩固组是 depth-1 的话题，它的摘要作为这个话题自己的一条原文、
  ...
  还有一条，而且一周之后它就是常态：**正文会老化出局，话题行不会**。一个巩固组的 `RoleDream` 摘要被保留窗扫掉之后，它在表面上就是一行没有正文的行，而它当初折进来的那几轮，活下来的正是各自的关键词轨——所以第（1）条是有条件的：父行还有正文时跳过 depth-2，父行没了正文就往那棵子树退回 `Keywords`，别把这份记忆整个丢掉。`Messages` 为空是「已过期的终局」，不是「读丢了一行」（`TestFusedGroupAgesIntoKeywordTracksNotSilence`）。
