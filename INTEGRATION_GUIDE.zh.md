...
| ContentRetentionMs | 604800000（7 天） | 一轮的记录（L4 内容与 L5 计划节点）保留多久后被 Dream 清走；0（没填）与负数都取库默认。没有「永不过期」的写法：要更长就把窗口调大，而不是关掉清扫。这个窗口量的是墙上时钟而不是轮数：钟比窗口还老的记录（一次回填、一份测试种子）会在第一次真跑起来的 Dream 里被扫掉，连被读到的机会都没有。**开着的一轮也不例外**——清扫读的是那条记录的钟，不是轮的状态；Dream 绝不做的是丢掉开着的那一轮，所以挂着的收束照样落得下（它自己写的两句是新钟，因此留下） |
...
- **把场景读折成进 prompt 的几行，是四个判定，这里一次说清。** `SceneContext("")` 交出的清单是
  **刻意摊平到 depth ≤ 2** 的：一个巩固组是 depth-1 的话题，它的摘要作为这个话题自己的一条原文、
  ...
  子的读，也是那些原文唯一还能回来的地方。于是：（1）`Depth > 1` 的行丢掉——那些是被某个表浅行吸收过的行：多半是一轮的原文，也可能是**被后一趟巩固折走的更早的一个组**
