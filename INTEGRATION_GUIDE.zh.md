...
| ContentRetentionMs | 604800000（7 天） | 一轮的记录（L4 内容与 L5 计划节点）保留多久后被 Dream 清走；0（没填）与负数都取库默认。没有「永不过期」的写法：要更长就把窗口调大，而不是关掉清扫。这个窗口量的是墙上时钟而不是轮数：钟比窗口还老的记录（一次回填、一份测试种子）会在第一次真跑起来的 Dream 里被扫掉，连被读到的机会都没有 |
...
- **把场景读折成进 prompt 的几行，是四个判定，这里一次说清。** `SceneContext("")` 交出的清单是
  **刻意摊平到 depth ≤ 2** 的：一个巩固组是 depth-1 的话题，它的摘要作为这个话题自己的一条原文、
  ...
  还有一条，而且一周之后它就是常态：**正文会老化出局，话题行不会**。一个巩固组的 `RoleDream` 摘要被保留窗扫掉之后，它在表面上就是一行没有正文的行，而它活下来的那部分就写在这一行自己身上：`Keywords` 是建组时从成员摘要里折出来的那一条，而话题行从不老化——被扫的从来只有正文。所以第（1）条的跳过是有条件的：组还有摘要时丢掉它的 depth-2 子行，摘要没了就把这一行自己的 `Keywords` 交出去，别把这份记忆整个丢掉。退回的东西必须在行上而不是隔一跳：摊平的那份列举不带父指针，读者走不进那棵子树。`Messages` 为空是「已过期的终局」，不是「读丢了一行」（`TestFusedGroupAgesIntoKeywordTracksNotSilence`）。
