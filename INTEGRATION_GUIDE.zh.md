...
| `db.MergeScenes(primaryID, []secondaryIDs) error` | 把几段对话并成一段：被吞掉的场景，其话题一律改挂到主场景下，它自己的场景记录随之不再。**留下的标题与 L3 锚定以主场景为准**——活下来的是先写的那个 id——而宿主在某一条轮次上写的标签在这次改挂中原样保留。被吞掉那一域的 L1 节点当场摘掉，主场景的节点由下一次 `Dream` 按它新拿到的轮次重建；域的记忆也一并挪过去，所以被吞掉那条场景上的轮次此后收不了口（节点与标签两头由 `TestInterfaceMemoryDeletionTakesItsNodeWithIt` 钉住） |
