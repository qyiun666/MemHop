...
export const DEFAULTS = {
  ...
  /** LLM/嵌入凭据 env 文件(KEY=VALUE 或 export KEY=VALUE)。 */
  ...
  /** 服务器监听端口(状态/健康探测用)。 */
  serverPort: 3939,
  /** 直接 spawn 时的嵌入模型与编码器地址(与 wrapper 一致)。 */
...
function wrapperTemplate(binPath, dbDir, embedModel, encoderAddr, port) {
  return `#!/bin/bash
set -a; source "${homeDir()}/.memhop/server.env"; set +a
exec "${binPath}" -db-dir "${dbDir}" -embed-model "${embedModel}" -encoder-addr "${encoderAddr}" -transport streamable-http -listen "127.0.0.1:${port}"
`;
}
...
class MemhopServerControl extends Service {
  ...
  async start() {
    ...
    const args = [
      "-db-dir", this.dbDir,
      "-embed-model", this.cfg.embedModel,
      "-encoder-addr", this.cfg.encoderAddr,
  ...
  installLaunchd() {
    ...
    try {
      writeFileSync(
        wrapper,
        wrapperTemplate(this.detectBin(), this.dbDir, this.cfg.embedModel, this.cfg.encoderAddr, this.port)
      );
