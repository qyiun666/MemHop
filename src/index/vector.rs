...
impl VectorPage {
    ...
    pub fn slot_size(&self) -> usize { 16 + self.dim * 2 }

    /// `(PAGE_SIZE - 32) / slot_size`, 32 bytes reserved for page metadata.
    pub fn vectors_per_page(&self) -> usize { (PAGE_SIZE - 32) / self.slot_size() }

    pub fn slot_offset(&self, slot_index: u16) -> usize { 32 + (slot_index as usize) * self.slot_size() }
}
...
unsafe fn cosine_similarity_avx2(a: &[f16], b: &[f16]) -> f32 {
    ...
    while i < len {
        let a_val = a[i].to_f32(); let b_val = b[i].to_f32();
        dot_product += a_val * b_val; norm_a += a_val * a_val; norm_b += b_val * b_val;
        i += 1;
    }
    let denom = (norm_a * norm_b).sqrt();
    if denom < 1e-10 { 0.0 } else { dot_product / denom }
}
...
unsafe fn cosine_similarity_neon(a: &[f16], b: &[f16]) -> f32 {
    ...
    while i < len {
        let a_val = a[i].to_f32(); let b_val = b[i].to_f32();
        dot_sum += a_val * b_val; norm_a_sum += a_val * a_val; norm_b_sum += b_val * b_val;
        i += 1;
    }
    let denom = (norm_a_sum * norm_b_sum).sqrt();
    if denom < 1e-10 { 0.0 } else { dot_sum / denom }
}
...
pub fn cosine_similarity(a: &[f16], b: &[f16]) -> f32 {
    ...
    #[cfg(target_arch = "x86_64")]
    { if is_x86_feature_detected!("avx2") && a.len() >= 8 { return unsafe { cosine_similarity_avx2(a, b) }; } }
    #[cfg(target_arch = "aarch64")]
    { if is_aarch64_feature_detected!("neon") && a.len() >= 4 { return unsafe { cosine_similarity_neon(a, b) }; } }
...
pub fn brute_force_knn(
    data: &[u8], query_vector: &[f16], btree: &BTreeIndex,
    vector_dim: usize, _page_count: u32, k: usize,
    ...
    for (&id_hash, &page_ref) in btree.iter() {
        ...
        if node_offset >= data.len() { continue; }
        if let Ok(node) = ContextNode::deserialize(&data[node_offset..]) {
            if node.vector_page_ref == 0 { continue; }
...
pub fn write_vector(
    mmap: &mut MmapMut, page_id: u32, slot_index: u16,
    id_hash: u64, vector: &[f16], dim: usize,
) -> IoResult<()> {
    ...
    if offset + slot_size > mmap.len() {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Write would exceed mmap bounds"));
    }
...
pub fn read_vector(data: &[u8], page_id: u32, slot_index: u16, dim: usize) -> IoResult<Vec<f16>> {
    ...
    if offset + slot_size > data.len() {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Read would exceed data bounds"));
    }
    let vector_start = offset + 16;
...
impl IVFIndex {
    pub fn new(dim: usize, initial_k: usize) -> Self {
        ...
        Self {
            centroids: Vec::with_capacity(cap), buckets: Vec::with_capacity(cap),
            dim, initial_k, k: 0,
            centroid_sums: Vec::with_capacity(cap), counts: Vec::with_capacity(cap),
        }
    }
    ...
    pub fn add_vector(&mut self, id_hash: u64, vector: &[f16], vec_page_id: u32, vec_slot_index: u16) {
        ...
        let idx = if self.centroids.is_empty() || self.centroids.len() < self.initial_k {
            ...
            self.centroid_sums.push(vector.iter().map(|x| x.to_f32()).collect());
            self.counts.push(1);
            ...
            for (i, &v) in vector.iter().enumerate() { sum[i] += v.to_f32(); }
            self.counts[best] += 1;
    ...
    pub fn rebuild_if_needed(&mut self, total_vectors: usize) {
        ...
        let target = self.initial_k.max((total_vectors as f64).sqrt().ceil() as usize).max(1);
        if target == self.k { return; }
    ...
    fn serialize_centroids(&self) -> Vec<u8> {
        ...
        bytes.push(IVF_VERSION); bytes.push(0); // flags
        bytes.extend_from_slice(&(self.dim as u16).to_le_bytes());
        bytes.extend_from_slice(&(self.k as u32).to_le_bytes());
        for c in &self.centroids { for v in c { bytes.extend_from_slice(&v.to_le_bytes()); } }
        bytes
    }
...
fn deserialize_centroids(bytes: &[u8]) -> MemHopResult<(usize, usize, Vec<Vec<f16>>)> {
    if bytes.len() < 12 || &bytes[0..4] != IVF_CLUSTER_MAGIC || bytes[4] != IVF_VERSION {
        return Err(MemHopError::Serialization("Invalid IVF centroid data".into()));
    }
    ...
    if bytes.len() < 12 + k * dim * 2 {
        return Err(MemHopError::Serialization("Truncated IVF centroid data".into()));
    }
    let mut centroids = Vec::with_capacity(k);
    let mut offset = 12;
    for _ in 0..k {
        let mut c = Vec::with_capacity(dim);
        for _ in 0..dim { c.push(f16::from_le_bytes([bytes[offset], bytes[offset + 1]])); offset += 2; }
        centroids.push(c);
    }
    Ok((dim, k, centroids))
}
...
fn deserialize_buckets(bytes: &[u8]) -> MemHopResult<(usize, IvfBuckets)> {
    ...
    let mut offset = 10;
    for _ in 0..k {
        if offset + 4 > bytes.len() { return Err(MemHopError::Serialization("Truncated IVF bucket header".into())); }
        ...
        for _ in 0..count {
            if offset + 14 > bytes.len() { return Err(MemHopError::Serialization("Truncated IVF bucket entry".into())); }
            ...
            bucket.push((id_hash, page_id, slot)); offset += 14;
        }
        buckets.push(bucket);
    }
    Ok((k, buckets))
...
pub fn ivf_knn(
    ivf: &IVFIndex, data: &[u8], query_vector: &[f16],
    k_results: usize, n_probes: usize,
) -> MemHopResult<Vec<(u64, f32)>> {
    if ivf.centroids.is_empty() || ivf.buckets.is_empty() || query_vector.len() != ivf.dim {
        return Ok(vec![]);
    }
    let probes = n_probes.min(ivf.k);
    if probes == 0 { return Ok(vec![]); }
    let mut centroid_scores: Vec<(usize, f32)> = ivf.centroids.iter().enumerate()
        .map(|(i, c)| (i, cosine_similarity(query_vector, c))).collect();
...
fn read_page_header_from_mmap(mmap: &MmapMut, page_id: u32) -> MemHopResult<PageHeader> {
    ...
    if offset + 32 > mmap.len() { return Err(MemHopError::PageNotFound(page_id)); }
...
fn read_page_data_from_mmap(mmap: &MmapMut, page_id: u32) -> MemHopResult<&[u8]> {
    let offset = page_id as usize * PAGE_SIZE;
    if offset + PAGE_SIZE > mmap.len() { return Err(MemHopError::PageNotFound(page_id)); }
    Ok(&mmap[offset + 32..offset + PAGE_SIZE])
}

fn read_u16_from_reserved(r: &[u8; 3988], o: usize) -> u16 { u16::from_le_bytes([r[o], r[o+1]]) }
fn read_u32_from_reserved(r: &[u8; 3988], o: usize) -> u32 { u32::from_le_bytes([r[o], r[o+1], r[o+2], r[o+3]]) }
fn write_u16_to_reserved(r: &mut [u8; 3988], o: usize, v: u16) { r[o..o+2].copy_from_slice(&v.to_le_bytes()); }
fn write_u32_to_reserved(r: &mut [u8; 3988], o: usize, v: u32) { r[o..o+4].copy_from_slice(&v.to_le_bytes()); }

fn allocate_and_write_chain(
    mmap: &mut MmapMut, header: &mut FileHeader, page_type: PageType,
    data: &[u8], file: &mut File,
) -> MemHopResult<u32> {
    let cap = PAGE_SIZE - 32;
    let n = if data.is_empty() { 1 } else { data.len().div_ceil(cap) };
    let mut pages = Vec::with_capacity(n);
    for _ in 0..n { pages.push(allocate_page(mmap, header, page_type, 0, EMPTY_FREE_LIST, file)?); }
    for (i, &pid) in pages.iter().enumerate() {
        let start = i * cap;
        let chunk = if start >= data.len() { &[] as &[u8] } else { &data[start..(start + cap).min(data.len())] };
...
fn free_page_chain(mmap: &mut MmapMut, header: &mut FileHeader, start_page: u32) -> MemHopResult<()> {
    let mut current = start_page;
    let mut visited = HashSet::new();
    while current != EMPTY_FREE_LIST && current != 0 {
        if !visited.insert(current) { break; }
        let next = read_page_header_from_mmap(mmap, current).map(|h| h.next_page).unwrap_or(EMPTY_FREE_LIST);
...
pub fn write_ivf_index(
    mmap: &mut MmapMut, header: &mut FileHeader, index: &IVFIndex, file: &mut File,
) -> MemHopResult<()> {
    ...
    if old_cr != 0 { free_page_chain(mmap, header, old_cr)?; }
    if old_br != 0 { free_page_chain(mmap, header, old_br)?; }
    let cluster_root = if index.centroids.is_empty() { 0 } else {
        allocate_and_write_chain(mmap, header, PageType::IVFCluster, &index.serialize_centroids(), file)?
    };
    let bucket_root = if index.buckets.is_empty() { 0 } else {
        allocate_and_write_chain(mmap, header, PageType::IVFBucket, &index.serialize_buckets(), file)?
    };
...
pub fn read_ivf_index(mmap: &MmapMut, header: &FileHeader) -> MemHopResult<Option<IVFIndex>> {
    ...
    if rd != dim || rk != k || rk != rk2 {
        return Err(MemHopError::Serialization("IVF persisted metadata mismatch".into()));
    }
    ...
    Ok(Some(IVFIndex { centroids, buckets, dim, initial_k: k, k, centroid_sums, counts }))
}
...
mod tests {
    ...
    fn test_cosine_similarity_partial() {
        ...
        assert!((sim - std::f32::consts::FRAC_1_SQRT_2).abs() < 0.01, "got {}", sim);
    }
    ...
    fn test_vector_read_write() {
        ...
        drop(f);
        let file = std::fs::OpenOptions::new().read(true).write(true).open(file.path()).unwrap();
        let mut mmap = unsafe { MmapMut::map_mut(&file).unwrap() };
        let original = vec![f16::from_f32(1.0), f16::from_f32(2.0), f16::from_f32(3.0), f16::from_f32(4.0)];
    ...
    fn test_brute_force_knn_empty() {
        ...
        assert!(brute_force_knn(data, &query, &btree, 3, 100, 10).unwrap().is_empty());
    }
    ...
    fn test_cosine_similarity_neon_aligned() {
        ...
        assert!((sim - expected).abs() < 1e-4, "SIMD {} vs fallback {}", sim, expected);
    }

    #[cfg(target_arch = "aarch64")]
    #[test]
    fn test_cosine_similarity_neon_path_directly() {
        if !is_aarch64_feature_detected!("neon") { return; }
        let a: Vec<f16> = (1..=5).map(|x| f16::from_f32(x as f32 * 0.1)).collect();
        let b: Vec<f16> = (1..=5).rev().map(|x| f16::from_f32(x as f32 * 0.1)).collect();
    ...
    fn test_ivf_index_add_and_assign() {
        ...
        let v1 = vec![f16::from_f32(1.0), f16::from_f32(0.0), f16::from_f32(0.0), f16::from_f32(0.0)];
        let v2 = vec![f16::from_f32(0.0), f16::from_f32(1.0), f16::from_f32(0.0), f16::from_f32(0.0)];
        let v3 = vec![f16::from_f32(0.9), f16::from_f32(0.1), f16::from_f32(0.0), f16::from_f32(0.0)];
    ...
    fn test_ivf_knn_matches_brute_force() {
        ...
        let vectors: Vec<(u64, Vec<f16>)> = vec![
            (1, vec![f16::from_f32(1.0), f16::from_f32(0.0), f16::from_f32(0.0), f16::from_f32(0.0)]),
            (2, vec![f16::from_f32(0.0), f16::from_f32(1.0), f16::from_f32(0.0), f16::from_f32(0.0)]),
            (3, vec![f16::from_f32(0.0), f16::from_f32(0.0), f16::from_f32(1.0), f16::from_f32(0.0)]),
            (4, vec![f16::from_f32(0.9), f16::from_f32(0.1), f16::from_f32(0.0), f16::from_f32(0.0)]),
            (5, vec![f16::from_f32(0.1), f16::from_f32(0.9), f16::from_f32(0.0), f16::from_f32(0.0)]),
        ];
        let file = NamedTempFile::new().unwrap();
        let mut f = std::fs::File::create(file.path()).unwrap();
        ...
        for (i, (id, vec)) in vectors.iter().enumerate() {
            ...
            let node = ContextNode {
                id_hash: *id, context_id: *id, vector_page_ref: encode_page_ref(vec_page, 0),
                importance: 1.0, valence: 0.0, arousal: 0.0, created_at: 0, updated_at: 0, version: 1, edge_ptrs: vec![],
            };
            let nb = node.serialize().unwrap();
            mmap[node_page as usize * PAGE_SIZE + 32..node_page as usize * PAGE_SIZE + 32 + nb.len()].copy_from_slice(&nb);
        ...
        let data: &[u8] = &mmap[..];
        let query = vec![f16::from_f32(1.0), f16::from_f32(0.0), f16::from_f32(0.0), f16::from_f32(0.0)];
    ...
    fn test_ivf_serialization_roundtrip() {
        ...
        f.write_all(&vec![0u8; PAGE_SIZE * 20]).unwrap();
        drop(f);
        let mut file = std::fs::OpenOptions::new().read(true).write(true).open(file.path()).unwrap();
        let mut mmap = unsafe { MmapMut::map_mut(&file).unwrap() };
        let mut header = FileHeader::new(dim as u16);
        init_free_list(&mut header).unwrap();
        for pid in (2..20).rev() { free_page(&mut mmap, &mut header, pid).unwrap(); }
        write_ivf_index(&mut mmap, &mut header, &ivf, &mut file).unwrap();
        let r = read_ivf_index(&mmap, &header).unwrap().expect("IVF present");
    ...
    fn test_ivf_rebuild_if_needed() {
        let mut ivf = IVFIndex::new(4, 2);
        let v = vec![f16::from_f32(1.0), f16::from_f32(0.0), f16::from_f32(0.0), f16::from_f32(0.0)];
        for id in 1..=16u64 { ivf.add_vector(id, &v, id as u32, 0); }
    ...
    fn test_ivf_accumulation_precision() {
        ...
        for i in 0..n {
            ...
            for (j, &val) in v.iter().enumerate() { true_sum[j] += val.to_f32(); }
        ...
        for (i, (a, e)) in ivf.centroids[0].iter().zip(expected_mean.iter()).enumerate() {
            assert!((a.to_f32() - e).abs() < 1e-2, "dim {} actual {} expected {}", i, a.to_f32(), e);
        }
    }
}
