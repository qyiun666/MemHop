...
impl PostingList {
    pub fn new() -> Self { Self { term_freq: HashMap::new(), doc_freq: 0 } }
}
...
fn is_stop_word(word: &str) -> bool { STOP_WORDS.contains(&word) }

fn tokenize_cjk(text: &str, keep_underscore: bool, filter_stop_words: bool) -> Vec<String> {
    jieba().cut(text, true).into_iter().map(|w| w.trim().to_lowercase())
        .map(|w| if keep_underscore {
            w.trim_matches(|c: char| !c.is_alphanumeric() && c != '_').to_string()
        } else {
...
pub fn tokenize(text: &str) -> Vec<String> {
    if has_cjk(text) { tokenize_cjk(text, false, true) }
    else {
        text.split_whitespace().map(|s| s.trim_matches(|c: char| !c.is_alphanumeric()).to_lowercase())
            .filter(|s| !s.is_empty() && !is_stop_word(s)).collect()
    }
}
...
pub struct SparsePageData {
    pub term_bucket_count: u32, pub doc_bucket_count: u32,
    pub term_count: u32, pub doc_count: u32,
    pub total_term_count: u64, pub avg_doc_length: f32,
    pub k1: f32, pub b: f32,
...
pub fn build_sparse_directory(page_data: &SparsePageData, term_starts: &[u32], doc_starts: &[u32], entity_start: u32) -> Vec<u8> {
    ...
    for &p in term_starts { dir.extend_from_slice(&p.to_le_bytes()); }
    for &p in doc_starts { dir.extend_from_slice(&p.to_le_bytes()); }
    dir
}

fn chunk_into_pages(data: &[u8]) -> Vec<Vec<u8>> { data.chunks(SPARSE_PAGE_PAYLOAD).map(|c| c.to_vec()).collect() }
...
fn unwrap_bucket_bytes(pages: &[Vec<u8>]) -> Result<Vec<u8>, String> {
    ...
    for page in pages { full.extend_from_slice(page); }
    if full.len() < 4 { return Err("Bucket data too short for length header".to_string()); }
    let len = u32::from_le_bytes([full[0], full[1], full[2], full[3]]) as usize;
    let end = 4 + len;
    if end > full.len() { return Err(format!("Bucket length header exceeds data: {} > {}", end, full.len())); }
...
#[derive(Serialize, Deserialize, Debug, Clone)]
struct BkNode { word: String, children: HashMap<usize, usize> }

#[derive(Serialize, Deserialize, Debug, Clone, Default)]
struct BkTree { nodes: Vec<BkNode> }

impl BkTree {
    fn new() -> Self { Self { nodes: Vec::new() } }

    fn insert(&mut self, word: String) {
        if self.nodes.is_empty() { self.nodes.push(BkNode { word, children: HashMap::new() }); return; }
    ...
    fn contains_recursive(&self, node_idx: usize, word: &str) -> bool {
        ...
        if let Some(&next_idx) = self.nodes[node_idx].children.get(&dist) { self.contains_recursive(next_idx, word) }
        else { false }
    }
    ...
    fn insert_recursive(&mut self, node_idx: usize, word: &str) {
        ...
        if let Some(&next_idx) = self.nodes[node_idx].children.get(&dist) {
            ...
            self.nodes.push(BkNode { word: word.to_string(), children: HashMap::new() });
    ...
    fn search_recursive(&self, node_idx: usize, word: &str, max_distance: usize, results: &mut Vec<(String, usize)>) {
        let node = &self.nodes[node_idx];
        let dist = levenshtein_distance(&node.word, word);
        if dist <= max_distance { results.push((node.word.clone(), dist)); }
        let min_dist = dist.saturating_sub(max_distance);
        let max_dist = dist + max_distance;
        for (&child_dist, &child_idx) in &node.children {
            if child_dist >= min_dist && child_dist <= max_dist { self.search_recursive(child_idx, word, max_distance, results); }
        }
    }
}
...
impl EntityIndex {
    pub fn new() -> Self { Self { entities: HashMap::new(), bk_tree: BkTree::new(), node_to_l2: HashMap::new() } }
    ...
    pub fn add_entity(&mut self, name: &str, node_hash: u64, l2_ids: Vec<u64>) {
        ...
        for l2_id in &l2_ids { if !entry.contains(l2_id) { entry.push(*l2_id); } }
    ...
    pub fn exact_match(&self, term: &str) -> Option<(u64, Vec<u64>)> { self.entities.get(&term.to_lowercase()).cloned() }

    pub fn fuzzy_match(&self, term: &str, max_distance: usize) -> Vec<(String, usize, u64, Vec<u64>)> {
        let mut results = Vec::new();
        for (word, dist) in self.bk_tree.search(term, max_distance) {
            if let Some(&(node_hash, ref l2_ids)) = self.entities.get(&word) { results.push((word, dist, node_hash, l2_ids.clone())); }
        }
        results
    }
    ...
    pub fn recognize_entities(&self, text: &str) -> Vec<(String, f32, Vec<u64>)> {
        ...
        for i in 0..words.len().saturating_sub(1) { tokens.push(format!("{} {}", words[i], words[i + 1])); }
        ...
        best_scores.into_iter().filter_map(|(word, score)| {
            self.entities.get(&word).map(|(_, l2_ids)| (word, score, l2_ids.clone()))
        }).collect()
    }
    ...
    pub fn build_from_l3(&mut self, data: &[u8], btree: &BTreeIndex) -> Result<Vec<(u64, String, Vec<String>)>, MemHopError> {
        ...
        for (&_id_hash, &page_ref) in btree.iter() {
            if page_type_of(data, page_ref) != Some(PageType::HypergraphNode as u16) { continue; }
            if let Some(slot_data) = get_slot_data(data, page_ref) {
                if let Ok(node) = HypergraphNode::deserialize(slot_data) {
                    ...
                    nodes_by_graph.entry(node.graph_id).or_default().push(info.clone());
                    all_nodes.push(info);
                }
            }
        }
        ...
        for (&_id_hash, &page_ref) in btree.iter() {
            if page_type_of(data, page_ref) != Some(PageType::Context as u16) { continue; }
            if let Some(slot_data) = get_slot_data(data, page_ref) {
                if let Ok(ctx) = ContextSlot::deserialize(slot_data) {
                    for &gh in &ctx.l3_refs { l2_by_graph.entry(gh).or_default().push(ctx.id_hash); }
                }
            }
        }
        for (graph_id, nodes) in nodes_by_graph {
            ...
            for (node_hash, title, keywords) in nodes {
                ...
                for kw in &keywords { self.add_entity(kw, node_hash, l2_ids.clone()); }
            }
        }
    ...
    pub fn l2_ids_for_node(&self, node_hash: u64) -> Vec<u64> { self.node_to_l2.get(&node_hash).cloned().unwrap_or_default() }
}

impl Default for EntityIndex { fn default() -> Self { Self::new() } }

fn tokenize_words(text: &str) -> Vec<String> {
    if has_cjk(text) { tokenize_cjk(text, true, false) }
    else {
        text.split_whitespace().map(|s| s.trim_matches(|c: char| !c.is_alphanumeric() && c != '_').to_lowercase())
...
impl SparseIndex {
    pub fn new() -> Self {
        Self { k1: 1.2, b: 0.75, postings: HashMap::new(), doc_lengths: HashMap::new(),
            avg_doc_length: 0.0, total_docs: 0, total_term_count: 0, entity_index: EntityIndex::new() }
    }

    pub fn with_params(k1: f32, b: f32) -> Self {
        Self { k1, b, postings: HashMap::new(), doc_lengths: HashMap::new(),
            avg_doc_length: 0.0, total_docs: 0, total_term_count: 0, entity_index: EntityIndex::new() }
    }

    /// For CJK text uses jieba-rs segmentation. No stop-word filtering.
    pub fn tokenize(text: &str) -> Vec<String> {
        if has_cjk(text) { tokenize_cjk(text, false, false) }
        else { text.split_whitespace().map(|s| s.to_lowercase()).collect() }
    }

    pub fn add_document(&mut self, id_hash: u64, terms: Vec<String>, doc_len: u32) {
        if self.doc_lengths.contains_key(&id_hash) { self.remove_document(id_hash); }
        ...
        for term in &terms { *term_freq_map.entry(term.clone()).or_insert(0) += 1; }
    ...
    pub fn remove_document(&mut self, id_hash: u64) {
        if let Some(&doc_len) = self.doc_lengths.get(&id_hash) {
            self.total_docs -= 1;
            self.total_term_count -= doc_len as u64;
            self.avg_doc_length = if self.total_docs > 0 { self.total_term_count as f32 / self.total_docs as f32 } else { 0.0 };
            for posting in self.postings.values_mut() {
                if posting.term_freq.remove(&id_hash).is_some() { posting.doc_freq -= 1; }
            }
    ...
    pub fn bm25_score(&self, query_terms: &[String], doc_id_hash: u64) -> f32 {
        let doc_len = match self.doc_lengths.get(&doc_id_hash) { Some(&l) => l as f32, None => return 0.0 };
        ...
        for term in query_terms {
            if let Some(posting) = self.postings.get(term) {
                if let Some(&tf) = posting.term_freq.get(&doc_id_hash) {
                    ...
                    let tf_n = (tf as f32 * (self.k1 + 1.0)) / (tf as f32 + self.k1 * (1.0 - self.b + self.b * doc_len / self.avg_doc_length));
                    score += idf * tf_n;
                }
            }
        }
    ...
    pub fn search(&self, query_terms: &[String], k: usize) -> Vec<(u64, f32)> {
        ...
        for term in query_terms {
            if let Some(posting) = self.postings.get(term) {
                for &doc_id in posting.term_freq.keys() { candidates.insert(doc_id); }
            }
        }
        let mut scores: Vec<(u64, f32)> = candidates.iter()
            .map(|&h| (h, self.bm25_score(query_terms, h))).filter(|(_, s)| *s > 0.0).collect();
        scores.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        scores.truncate(k);
        scores
    }

    pub fn build_entity_index(&mut self, data: &[u8], btree: &BTreeIndex) -> Result<(), MemHopError> {
        let l3_nodes = self.entity_index.build_from_l3(data, btree)?;
        for (node_hash, title, keywords) in l3_nodes {
            let doc_terms: Vec<String> = std::iter::once(title).chain(keywords)
                .flat_map(|s| SparseIndex::tokenize(&s)).collect();
    ...
    pub fn entity_search(&self, query: &str) -> Vec<(u64, f32)> {
        ...
        for (_, score, l2_ids) in entities { for l2_id in l2_ids { *scores.entry(l2_id).or_insert(0.0) += score; } }
    ...
    pub fn entity_search_nodes(&self, query: &str) -> Vec<(u64, Vec<u64>)> {
        ...
        for word in &words {
            for (_, _, node_hash, l2_ids) in self.entity_index.fuzzy_match(word, 2) { seen.entry(node_hash).or_insert(l2_ids); }
        }
        seen.into_iter().collect()
    }

    pub fn has_entity_index(&self) -> bool { !self.entity_index.is_empty() }
    pub fn entity_index(&self) -> &EntityIndex { &self.entity_index }
    pub fn len(&self) -> usize { self.total_docs as usize }
    ...
    pub fn top_terms(&self, n: usize) -> Vec<(String, u32)> {
        let mut tf: Vec<(String, u32)> = self.postings.iter().map(|(t, p)| (t.clone(), p.doc_freq)).collect();
        tf.sort_by_key(|b| std::cmp::Reverse(b.1));
        tf.truncate(n);
        tf
    }

    pub fn serialize(&self) -> Result<Vec<u8>, String> { bincode::serialize(self).map_err(|e| format!("Serialization failed: {}", e)) }
    pub fn deserialize(data: &[u8]) -> Result<Self, String> { bincode::deserialize(data).map_err(|e| format!("Deserialization failed: {}", e)) }
    ...
    pub fn serialize_to_pages(&self) -> Result<SparsePageData, String> {
        let term_bc = if self.postings.is_empty() { 0 } else { SPARSE_BUCKET_COUNT };
        let doc_bc = if self.doc_lengths.is_empty() { 0 } else { SPARSE_BUCKET_COUNT };
        let mut term_buckets: Vec<Vec<Vec<u8>>> = Vec::new();
        if term_bc > 0 {
            let mut buckets: Vec<Vec<(String, PostingList)>> = vec![Vec::new(); term_bc as usize];
            for (term, posting) in &self.postings { buckets[(hash_id(term) % term_bc as u64) as usize].push((term.clone(), posting.clone())); }
            for bucket in buckets {
                let bytes = if bucket.is_empty() { Vec::new() } else { bincode::serialize(&bucket).map_err(|e| format!("Term bucket serialization failed: {}", e))? };
        ...
        if doc_bc > 0 {
            ...
            for (&doc_id, &len) in &self.doc_lengths { buckets[(doc_id % doc_bc as u64) as usize].push((doc_id, len)); }
            for bucket in buckets {
                let bytes = if bucket.is_empty() { Vec::new() } else { bincode::serialize(&bucket).map_err(|e| format!("Doc bucket serialization failed: {}", e))? };
                doc_buckets.push(wrap_and_chunk(&bytes));
            }
        }
        let entity_chain = if self.entity_index.is_empty() { Vec::new() }
        else { wrap_and_chunk(&bincode::serialize(&self.entity_index).map_err(|e| format!("Entity index serialization failed: {}", e))?) };
        Ok(SparsePageData { term_bucket_count: term_bc, doc_bucket_count: doc_bc,
            term_count: self.postings.len() as u32, doc_count: self.doc_lengths.len() as u32,
            total_term_count: self.total_term_count, avg_doc_length: self.avg_doc_length,
            k1: self.k1, b: self.b, term_buckets, doc_buckets, entity_chain })
    }

    pub fn deserialize_from_pages(page_data: &SparsePageData) -> Result<Self, String> {
        if page_data.term_buckets.len() != page_data.term_bucket_count as usize { return Err(format!("Term bucket count mismatch: expected {}, got {}", page_data.term_bucket_count, page_data.term_buckets.len())); }
        if page_data.doc_buckets.len() != page_data.doc_bucket_count as usize { return Err(format!("Doc bucket count mismatch: expected {}, got {}", page_data.doc_bucket_count, page_data.doc_buckets.len())); }
        let mut postings: HashMap<String, PostingList> = HashMap::with_capacity(page_data.term_count as usize);
        for bp in &page_data.term_buckets {
            let bytes = unwrap_bucket_bytes(bp)?;
            if bytes.is_empty() { continue; }
            for (term, posting) in bincode::deserialize::<Vec<(String, PostingList)>>(&bytes).map_err(|e| format!("Term bucket deserialization failed: {}", e))? { postings.insert(term, posting); }
        }
        let mut doc_lengths: HashMap<u64, u32> = HashMap::with_capacity(page_data.doc_count as usize);
        for bp in &page_data.doc_buckets {
            let bytes = unwrap_bucket_bytes(bp)?;
            if bytes.is_empty() { continue; }
            for (doc_id, len) in bincode::deserialize::<Vec<(u64, u32)>>(&bytes).map_err(|e| format!("Doc bucket deserialization failed: {}", e))? { doc_lengths.insert(doc_id, len); }
        }
        let mut entity_index = if page_data.entity_chain.is_empty() { EntityIndex::new() }
        else { bincode::deserialize(&unwrap_bucket_bytes(&page_data.entity_chain)?).map_err(|e| format!("Entity index deserialization failed: {}", e))? };
        // Rebuild reverse index (marked #[serde(skip)])
        entity_index.rebuild_node_to_l2();
        Ok(Self { k1: page_data.k1, b: page_data.b, postings, doc_lengths, avg_doc_length: page_data.avg_doc_length,
            total_docs: page_data.doc_count, total_term_count: page_data.total_term_count, entity_index })
    }
}

impl Default for SparseIndex { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn create_test_mmap(pages: usize) -> MmapMut {
        ...
        drop(f);
        let f = std::fs::OpenOptions::new().read(true).write(true).open(tf.path()).unwrap();
    ...
    fn create_test_context(id_hash: u64, title: &str, l3_refs: Vec<u64>) -> ContextSlot {
        ContextSlot { id_hash, parent_id: None, depth: 1, title: title.to_string(), summary: None,
            archive_refs: Vec::new(), l3_refs, turn_count: 0, created_at: 0, updated_at: 0,
            version: 1, importance: 0.5, activation_score: 0.0, is_active: true,
            activation_state: ActivationState::Active, centroid_page_ref: 0, dialogue_range: (0, 0),
            llm_params: crate::slot::context::LlmParams::default() }
    }

    #[test]
    fn test_tokenize() { assert_eq!(SparseIndex::tokenize("Hello World hello"), vec!["hello", "world", "hello"]); }
    ...
    #[test]
    fn test_tokenize_mixed() { assert_eq!(SparseIndex::tokenize("Hello 人工智能 world"), vec!["hello", "人工智能", "world"]); }
    ...
    fn test_bm25_idf_rare_term() {
        let mut idx = SparseIndex::new();
        for i in 0..10 { idx.add_document(i, vec!["common".to_string()], 1); }
    ...
    fn test_search_top_k() {
        let mut idx = SparseIndex::new();
        for i in 0..5 { let t = SparseIndex::tokenize(&format!("document number {}", i)); idx.add_document(i as u64, t.clone(), t.len() as u32); }
    ...
    fn test_bm25_formula_verification() {
        ...
        assert!(score > 0.25 && score < 0.35, "score should be ~0.288, got {}", score);
    }
    ...
    fn test_top_terms_basic() {
        ...
        idx.add_document(3, vec!["rust".to_string(), "programming".to_string(), "language".to_string()], 3);
    ...
    fn test_bk_tree_duplicate_insertion() {
        let mut tree = BkTree::new();
        for _ in 0..100 { tree.insert("apple".to_string()); }
    ...
    fn test_entity_index_exact_match() {
        ...
        assert_eq!(idx.exact_match("rust programming"), Some((101, vec![1001, 1002])));
        assert_eq!(idx.exact_match("RUST PROGRAMMING"), Some((101, vec![1001, 1002])));
    }
    ...
    fn test_node_to_l2_survives_serialization_roundtrip() {
        ...
        idx.entity_index.add_entity("ownership", 1000, vec![10, 11, 12]);
        let pd = idx.serialize_to_pages().unwrap();
        let d = SparseIndex::deserialize_from_pages(&pd).unwrap();
    ...
    fn test_build_entity_index_from_l3() {
        ...
        let node = HypergraphNode { id_hash: 1001, graph_id: 1000, title: "Rust Programming".to_string(),
            node_type: "concept".to_string(), content: "A systems programming language".to_string(),
            keywords: vec![], source_ref: None, importance: 0.9, created_at: 0, updated_at: 0, version: 1 };
