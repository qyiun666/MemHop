...
/// IPC-based encoder (placeholder for v0.31+)
#[allow(dead_code)]
pub struct IpcEncoder {
    socket_path: PathBuf, // Kept for future IPC implementation
    dim: usize,
    mode: String,
}
...
impl Encoder for MockEncoder {
    fn encode(&self, text: &str) -> EncoderOutput {
        // Generate deterministic mock vectors based on text hash
        let hash = text.len() as f32;

        let dense = (0..self.dim)
            .map(|i| f16::from_f32((hash + i as f32) / (self.dim as f32)))
            .collect();
