...
impl MemHop {
    ...
    pub fn get_l5(&self, id: &str) -> Result<Option<CrystalSummary>> {
        Ok(
            crate::query::l5_ops::get_l5(&self.mmap, &self.btree, id)?.map(|c| CrystalSummary {
    ...
    pub fn update_l5(&mut self, id: &str, fields: UpdateL5Fields) -> Result<CrystalSummary> {
        crate::query::l5_ops::update_l5(&mut self.mmap, &mut self.header, &self.btree, id, fields)?;
        self.get_l5(id).map(|opt| opt.expect("chain just updated"))
    }

    /// Delete an L5 action chain and all its steps.
    pub fn delete_l5(&mut self, id: &str) -> Result<()> {
        crate::query::l5_ops::delete_l5(&mut self.mmap, &mut self.header, &mut self.btree, id)
    }
}
