...
impl ContextNode {
    ...
    pub fn slot_size(&self) -> usize { 66 + self.edge_ptrs.len() * 8 }
    ...
    pub fn serialize(&self) -> io::Result<Vec<u8>> {
        ...
        for &ptr in &self.edge_ptrs { buf.write_all(&ptr.to_le_bytes())?; }
        Ok(buf)
    }
    ...
    pub fn deserialize(data: &[u8]) -> io::Result<Self> {
        ...
        for _ in 0..edge_count { edge_ptrs.push(read_u64(&mut c)?); }
        Ok(ContextNode {
            id_hash, context_id, vector_page_ref, importance,
            valence, arousal, created_at, updated_at, version, edge_ptrs,
        })
    }
}
...
fn read_f64(c: &mut Cursor<&[u8]>) -> io::Result<f64> { let mut b = [0u8; 8]; c.read_exact(&mut b)?; Ok(f64::from_le_bytes(b)) }
...
mod tests {
    ...
    fn test_context_node_roundtrip() {
        let node = ContextNode {
            id_hash: 12345, context_id: 67890, vector_page_ref: 42,
            importance: 0.85, valence: 0.0, arousal: 0.0,
            created_at: 1000000, updated_at: 2000000, version: 1,
    ...
    fn test_context_node_empty_edges() {
        let node = ContextNode {
            id_hash: 1, context_id: 2, vector_page_ref: 0,
            importance: 0.0, valence: 0.0, arousal: 0.0,
            created_at: 0, updated_at: 0, version: 0, edge_ptrs: vec![],
        };
        assert_eq!(node.serialize().unwrap().len(), 66);
        assert_eq!(node, ContextNode::deserialize(&node.serialize().unwrap()).unwrap());
    }

    #[test]
    fn test_context_node_slot_size() {
        let node = ContextNode {
            id_hash: 1, context_id: 2, vector_page_ref: 3,
            importance: 0.5, valence: -0.3, arousal: 0.7,
            created_at: 0, updated_at: 0, version: 0, edge_ptrs: vec![10, 20],
        };
