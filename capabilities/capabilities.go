...
// crystallize and import. The files are embedded so both the Go library
// and the memhop-mcp binary can load them without any on-disk path.
//
...
// (ListCapabilities / GetCapability); see memhop.BuiltinCapabilityFS. They
// are read-only: lifecycle writes (Activate/Usage/Delete/Update) reject them.
//
//go:embed *.json
var FS embed.FS
