...
| `db.PlanCommit(topicID, nodePath, ev, api.PlanStep{Title: "research", Status: api.PlanStatusDone, Summary: s})` | commit one step: advance the node's status, append its event, and fold a parent's Summary out of its children once **every** child has reached a final state (a parent turns `done` only when the host commits it, and a fold over still-open children would read as a finished answer). A `nodePath` missing along the dotted path is created as pending — this is how a step is added. A blank `Title`/`Summary` keeps what the node already holds; an unknown `Status` is refused before the tree moves |
