...
| 分组 | 方法 |
|------|------|
| 核心循环 | `Search(q) → topicID` · `PlanSet(topicID, []PlanStep{…})`（计划先于步骤） · `AppendArchive(topicID, ArchiveSlot{...})` · `Update(sceneID, topicID)` · `Dream(ctx, sceneID)` |
