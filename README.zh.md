...
4. **L0 蒸馏** — 从排序后的 L1 样本蒸馏情绪/MBTI 与一份人格摘要，合入存量画像（宿主自有的那几项原样保留），同一份回包带回的逐节点情感回填到还没有情感的 L1 节点上；没有样本可读时整段跳过
