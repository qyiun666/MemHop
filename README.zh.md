...
| 版本 | 日期                 | 亮点 | 核心改动 |
|------|----------------------|------|---------|
| v1.6.6 | 2026-09-23 | 一轮只剩一次收束：域自持场景与轮次，写入形状不再收归属字段 | 1. **`Settle` 退役**：一次 `Update(TurnEnd{Input,Output,Outcome,CreatedAt})` 收束一轮——Input/Output 原地覆写该轮话题的 Seq 1/Seq 2，Outcome 按调用次数追加成 `turn_outcome` 事件（一次挂起＋一次恢复是两条事实）。2. **场景与轮次两个 id 由域自持**：`Search(SearchQuery{})` 续用该域当前场景并开出这一轮（`NewScene:true` 才另开一条会话，重启靠场景原有的 `turn_seq` 恢复），`SceneContext("")` 是纯读、不吃轮次、也不替没读过的域新建场景（`ErrNotFound`）——只有开轮那次读前进计数，带工具调用的循环不会替没发生过的轮白跳。3. 锚点只由会新建场景的那一读采纳，续用时递 `L3ID` 一律拒。4. **写形状与读形状分开**：`AppendArchive` 收 `ArchiveInput`，读回形状上那两个归属字段在写侧**没有位置**（与 `ProfileInput` 同一条标准），「写到宿主没在做的轮上」被形状关死；没有开着轮即拒写入。5. `DeleteScene`/`DeleteTopic`/`MergeScenes` 同步摘掉自持字段，已毁的轮不可能被下一次 `Update` 继续写；轮次键撞上保留的全零值直接报损坏。6. 宿主的 hex id 只在一处跨界（`internal.FormatID`/`internal.parseID`），第 3 层以下不再收 id 字符串；公开面维持 `Session` 25 + `DB` 7。7. 毫秒在写边界判（秒级与微秒级值一律拒）——跨仓端到端实测之后，宿主侧不再有单位换算。磁盘格式 `0x0012` 不变 |
