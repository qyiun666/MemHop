...
- **L1 场景超图** — Dream 在关键词集合重叠的场景间创建共现超边（Jaccard ≥ `L1EdgeMinSimilarity`）并按时间衰减剪枝；一条边以建边时那个相似度定重，此后只淡出——只有某一端的关键词真的变了才会重新加权，把一对没动过的场景再测一遍不算重新经历。L1 由 Dream 维护，供显式图查询与后续关联消费——读取路径不打分、不扩散
