...
- **V2 追加写入存储** — `.meh` 格式（`FormatVersion=0x000B`），A/B 双头 + 记录级 CRC32 + 撕裂尾帧截断恢复，mmap 零拷贝读取，快照/检查点。记录帧携带 8 字节 `agent_id`（26 字节帧头），引擎按 `(agent, idHash)` 域索引全部记录。L3 知识图与 L5 能力卡记录驻留文件级保留公共域。**仅认 `0x000B`**——更早的 `.meh` 数据文件 Open 时显式拒绝，无迁移路径
...
- **MCP Server** — `cmd/memhop-mcp` 将 32 个公开会话方法中的 26 个以 MCP 工具通过多租户 HTTP 暴露（SSE + streamable-http，官方 `modelcontextprotocol/go-sdk`）：单进程服务多个宿主，共享一个 `.meh` 文件，每个租户按 URL 路径 `/mcp/<tenant-id>` 隔离到独立 agent 域（租户名 → 稳定 agentID，`os.Root` 锚定 db 目录，`memhop_capability_import` 的路径再锚定到 `--capability-dir`，缺省即 db 目录；L3 知识图与 L5 能力池是全租户共享的两个池）。刻意只留在 Go 侧：L6 计划写读面（`PlanCommit`/`PlanState`/`SyncPlanTree`）、记忆纠错（`DeleteTopic`/`DeleteScene`/`DeleteL3Nodes`）与文件维护（`CompactTo`，入参就是一个输出路径）——这些要由持有会话状态、或该决定文件写到哪里的宿主来调
...
| L5 能力 | `ImportCapability(path)`（导入 v4 包） · `UpdateCapability`（`status` 补丁即激活） · `DeleteCapability` · `ListCapabilities(q)` · `RecordCapabilityUsage` |
...
引擎说明书卡由 `internal.BuiltinCards()` 在 Open 时**代码组装**，共 **9 张**（英文）：`memhop-guide`（循环分工总纲——Search/Update/Dream 与轨迹记录由宿主自动执行、LLM 勿手动调用——外加其余八张卡的索引）+ 八张说明书（cycle、knowledge、scene、archive、profile、capability、trajectory、plan），**覆盖全部 32 个会话方法**。一张卡 = **名称 + N 个功能条目**——每个条目（`ResourceRef`）自带启动方式（`type: mcp|skill|api|composite` + `ref`/`config`）、说明（`desc`）与用法（`input` JSON Schema / `output`），卡片级没有 type；动作链就是 `composite` 条目 `config` 里的 `{"steps":[{"tool":"...","args":{...}}]}`。说明书卡描述 Go API 调用契约（`type: "api"`、`ref: "api:MethodName"`），宿主直接调用，无需 MCP 层。**资源即工具声明**：`name/desc/input/output` 与宿主工具规格（meowire `ToolSpec`）字段完全同构，宿主纯字段拷贝即可投影、零格式转换。**分层注入**：说明书卡**只列不存**（不落 `.meh` 文件，`Search` 响应不附带）——`ListCapabilities` 按与库存能力同一套过滤器返回它们（另加 `Package` 按包过滤），对内置卡 id 的写入一律 `ErrNotFound`，同名能力按 ID 去重库存优先（同名字卡入库即遮蔽内置卡，删除即还原；结晶折回拒绝撞名说明书）；默认只投影一行一卡索引（`id + name + summary + trigger`）+ guide 卡，参数详情按需 `ListCapabilities(CapabilityListQuery{IDs: []string{id}})` 获取。**L5 与 L3 一样是文件级公共池**：能力记录住保留共享域，所有 agent 共用全部卡；`.meh` 同目录下的 `plug/`（`plug/<包名>/capability.json`）在 Open 时自动导入——坏包 Warn 跳过，不阻断。
...
| 版本 | 日期                 | 亮点 | 核心改动 |
|------|----------------------|------|---------|
| v1.7.0 | 2026-09-06 | 公开面再收敛（34→32）；内置说明书卡改为代码组装（6→9 张，覆盖全部会话方法） | 1. **`ActivateCapability` 删除**：repo 层的激活本就是「读卡→置 active→写回」，与 `UpdateCapability(id, CapabilityPatch{Status: &active})` 的状态补丁完全重合——同一能力只留一个实现，激活语义并入 `UpdateCapability`（并入后同过卡片校验，夹具缺 summary/resources 的裸卡激活会被拒）<br>2. **`PlanReplace` 删除**：`SyncPlanTree(planID, nil)` 清整树（节点与绑定事件全删、保留 planID、事件 Seq 归 1），播种下一个任务 = 单节点同步（空 status 落 pending）；`0000000000000000` 拒绝语义不变（ParsePlanID 在 nil 判断之前）<br>3. **MCP 31 → 30 工具**：`memhop_capability_activate` 并入 `memhop_capability_update` 的 `status` 参数<br>4. **内置说明书卡改为代码组装**：`capabilities/` 内嵌包整删（`internal.Open` 去掉 `fs.FS` 注入参数），`internal.BuiltinCards()` 直接构建 9 张卡（guide 总纲 + cycle/profile/scene/knowledge/archive/capability/trajectory/plan），**覆盖全部 32 个会话方法**——原先没有说明书卡的核心循环（Search/Update/Dream）、轨迹三方法与计划树三方法补齐；卡仍为稳定名派生 id、active、`Origin=builtin`，**只列不存**——对内置卡 id 的写一律 `ErrNotFound`（四处散布的只读守卫删除），同名字卡入库即遮蔽（stored wins，删除即还原），结晶折回仍拒绝撞名说明书（`findBuiltinCapability` 唯一保留点）；`plug/` 自动注入机制不变<br>5. 无格式变更（仍 `0x000B`），无需迁移；打 tag 后 meowagent 侧需跟版适配（`ActivateCapability`/`PlanReplace` 两个调用点） |
