...
| `AppendArchive(topicID, ArchiveSlot{Kind, ...})` | 一轮内容的唯一写入面：原文声明谁说的、是什么媒介；事件自己命名，并可挂在某个计划步骤上。`Seq: 0` 在两个对话槽之上分配 | 零 LLM；被拒的记录一字节不留（含顺路要建的节点）。事件整条 4 KiB（名字与正文合计）、原文 64 KiB，超预算是拒写不是截断 |
