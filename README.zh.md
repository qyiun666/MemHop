...
<p align="center">
  <strong>当前版本：v1.7.0（未发布）· 最新稳定 tag：v1.6.0</strong>
</p>
...
- **V2 追加写入存储** — `.meh` 格式（`FormatVersion=0x000B`），A/B 双头 + 记录级 CRC32 + 撕裂尾帧截断恢复，mmap 零拷贝读取，快照/检查点。记录帧携带 8 字节 `agent_id`（26 字节帧头），引擎按 `(agent, idHash)` 域索引全部记录。**与 `0x000A`（及更早）的 `.meh` 数据文件不兼容**——Open 时显式拒绝，无迁移路径（0x000A 把 L3 知识图记录、0x000B 把 L5 能力卡记录先后移入文件级保留公共域；v1.5.0 的话题单轨化未 bump 版本，旧库的 `user_keywords`/`agent_keywords` 在解码点归一进 `fused_keywords`）
- **多 Agent 域** — `OpenMulti` + `CreateAgent(name)` / `Session(agentID)` / `ListAgents` / `DeleteAgent`：多个 agent 共享一个 `.meh` 文件，各自拥有完全隔离的域（话题缓存、Dream 管线、域级锁）；同 agent 串行、跨 agent 并行；空闲域按访问节奏回收内存（`Defaults.AgentIdleTTLMs`），记录仍在文件。多 agent 是唯一模式——所有操作都经由按域绑定的会话执行。例外是 L3 与 L5（见下）：知识图与能力池都是文件级公共池
...
> **并发契约。** 同一 agent 的操作（Search / Update / Dream / 写 API）由库内域级锁串行，跨 agent 在 `*MultiAgentDB` 上并行，宿主无需自行排队。`*memhop.Session` 除绑定的域 ID 外不携带任何跨域状态。文件排他锁仍保证一个 `.meh` 文件只能被一个进程打开；`*MultiAgentDB` 不再暴露任何锁接口（v1.5.0 已删）——域锁是库的，宿主自己的临界区请自行加锁。
...
| L5 能力 | `ImportCapability(path)`（导入 v4 包） · `UpdateCapability` · `DeleteCapability` · `ListCapabilities(q)` · `ActivateCapability` · `RecordCapabilityUsage` |
...
| L6 计划树 | `PlanCommit` · `PlanState` · `PlanReplace` · `SyncPlanTree`，计划 id 由 `api.NewPlanID(name)` 铸造（仅 Go module 暴露，MCP 工具集未接入） |
...
仓库根目录 `capabilities/` 内置 **6 张能力卡**（`memhop-capability/v4` 单卡包格式，构建时随库内嵌，英文）：`memhop-guide`（循环分工总纲——Search/Update/Dream 与轨迹记录由宿主自动执行、LLM 勿手动调用——外加其余五张卡的索引）+ 五张 LLM 可调用说明书（knowledge、scene、archive、profile、capability）。一张卡 = **名称 + N 个功能条目**——每个条目（`ResourceRef`）自带启动方式（`type: mcp|skill|api|composite` + `ref`/`config`）、说明（`desc`）与用法（`input` JSON Schema / `output`），卡片级没有 type；动作链就是 `composite` 条目 `config` 里的 `{"steps":[{"tool":"...","args":{...}}]}`。说明书卡描述 Go API 调用契约（`type: "api"`、`ref: "api:MethodName"`），宿主直接调用，无需 MCP 层。**资源即工具声明**：`name/desc/input/output` 与宿主工具规格（meowire `ToolSpec`）字段完全同构，宿主纯字段拷贝即可投影、零格式转换。**分层注入**：`ListCapabilities` 只读返回内置卡（与库存能力同套过滤器，另加 `Package` 按包过滤，不落 `.meh` 文件，同名能力按 ID 去重库存优先，`Search` 响应不附带）；默认只投影一行一卡索引（`id + name + summary + trigger`）+ guide 卡，参数详情按需 `ListCapabilities(CapabilityListQuery{IDs: []string{id}})` 获取。**L5 与 L3 一样是文件级公共池**：能力记录住保留共享域，所有 agent 共用全部卡；`.meh` 同目录下的 `plug/`（`plug/<包名>/capability.json`）在 Open 时自动导入——坏包 Warn 跳过，不阻断。
...
| 版本 | 日期                 | 亮点 | 核心改动 |
|------|----------------------|------|---------|
| Unreleased（v1.7.0） | 2026-09-06 | L5 统一卡 + 能力文件级公共池 + `plug/` 自动注入 | 1. **一卡一形态**：卡 = 名称 + N 个功能条目，每条目自带启动方式（`type: mcp|skill|api|composite` + `ref`/`config`）、说明（`desc`）、用法（`input` JSON Schema/`output`）；卡片级 `Type`/`Workflow` 删除——动作链归宿到 `composite` 条目的 `config`（`{"steps":[{"tool":...}]}`，键名与宿主可执行解析器对齐）<br>2. **`memhop-capability/v4` 插件包文档**：`{format, name, capabilities:[1..N]}`，一个文件 = 一个插件包，包名随卡落 `Package` 字段；卡名跨包全局唯一（id = 名字哈希，撞名显式拒绝）<br>3. **L5 升级为文件级公共池**（与 L3 同机制：记录迁入保留共享域，`SharedL3AgentID` 更名 `SharedPoolAgentID`，域值不变）；格式 0x000A → 0x000B，旧文件 Open 拒绝不迁移<br>4. **`<meh 同目录>/plug/` 自动注入**：每个含 `capability.json` 的子文件夹在 Open 时导入共享池，坏包 Warn（路径+原因）跳过不阻断；字节级不变的重注入零写入<br>5. `ImportCapability(path)` 升级为整包导入，返回逐卡处置（created/updated/errors）；`CapabilityListQuery` 新增 `Package` 过滤；MCP 工具面不变（31）；内置 6 卡重写为 v4 单卡包 |
| v1.6.0 | 2026-09-04 | 接口去 fallback；补齐 v1.5.0 公开面 | v1.5.0 tag 打在了 API 面完成之前——require 它会报 `undefined: api.DefaultAgentID / api.NewPlanID / api.ScenePatch`，本版本交付补齐后的完整线。1. **ID 一律库内发号**：`api.DefaultAgentID` 指隐式域，`api.NewPlanID(name)` 铸计划 id，`api/ids.go` 四个 Format/Parse 桥删除<br>2. **公开面合并**：`SetSceneName`+`SetSceneL3ID` → `UpdateScene(id, ScenePatch)`、`PlanAppend` → `AppendTrajectory(key, nodePath, ev)`、`ListScenesByL3` → `ListScenes(l3ID)`；删除 `Lock`/`Unlock`/`Session.Checkpoint`/`IsClosed`/`AgentID`/`DistillL0`/`ListPlans`/`GetArchive`/`GetCapability`（现为 34 + 8）<br>3. **接口不允许 fallback，有问题就返回 error**：LLM 关键词输出不可解析即 `ErrLLM`、该轮零留痕（gse/分词兜底删除，直接依赖 5 → 4）；被拒的 `PlanCommit` 树与事件零变化；超预算轨迹 payload 拒绝而非截断；`DeleteCapability`/`DeleteAgent`/`MergeScenes` 对未知 id 报错而非静默成功；读路径瞬时错误一律上报不再跳条<br>4. **图导入闭环**：Skip 重导补建边、既有图槽复用（改名在重导后存活）、整批先校验、`L3Relation.Titles` 声明真 N 元超边、`UpdateL3` 撞名拒绝且重导路由确定、删图级联清场景锚点、`SyncPlanTree` 删分支镜像进轨迹索引<br>5. **格式版本仍 0x0009**，MCP 工具面不变（31）；每条修复都以把本仓导进自身 L3 超图实测过，逐条记录见 CHANGELOG |
