...
| L5 Capability | Directory-as-capability, no stored records: `ParseCapabilityPackage(data, source)` · `ValidateCapabilityCard(card)` (package-level v4 parse/validate) · `Crystallize(key, existing)` returns candidates — persisting them is the host's job |
