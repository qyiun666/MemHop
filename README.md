...
- **V2 Storage** — `.meh` format (`FormatVersion=0x000B`) with A/B dual headers, per-record CRC32 + torn-write truncation recovery, mmap zero-copy, snapshot/checkpoint. Record frames carry an 8-byte `agent_id` (26-byte header) and the engine indexes every record by `(agent, idHash)` domain. The L3 knowledge graph and the L5 capability cards live in the file-wide reserved shared domain. **Not compatible with `0x000A` (or older) `.meh` data files** — they are rejected at Open with no migration path
- **Multi-Agent Domains** — `OpenMulti` + `CreateAgent(name)` / `Session(agentID)` / `ListAgents` / `DeleteAgent`: many agents share one `.meh` file with fully isolated per-agent domains (caches, Dream pipelines, domain locks); same-agent operations serialize, different agents run in parallel; idle domains reclaim memory on access cadence (`Defaults.AgentIdleTTLMs`) while their records stay on disk. Multi-agent is the only mode — every operation runs through a per-domain session. Two exceptions (below): the L3 knowledge graph and the L5 capability pool are file-wide shared
...
- **MCP Server** — `cmd/memhop-mcp` exposes 26 of the 32 public session methods as MCP tools over multi-tenant HTTP (SSE + streamable-http, official `modelcontextprotocol/go-sdk`): one process serves many hosts through one shared `.meh` file, each tenant isolated by URL path `/mcp/<tenant-id>` into its own agent domain (stable agentID per tenant name, `os.Root`-anchored db-dir, and `memhop_capability_import` paths anchored to `--capability-dir`, defaulting to the db-dir; the L3 knowledge graph and the L5 capability pool are the two pools all tenants share). Go-only by design: the L6 plan write/read surface (`PlanCommit`/`PlanState`/`SyncPlanTree`), memory correction (`DeleteTopic`/`DeleteScene`/`DeleteL3Nodes`) and file maintenance (`CompactTo`, whose argument is an output path) — those need a host that owns session state or chooses where a file is written
...
| L5 Capability | `ImportCapability(path)` (v4 package) · `UpdateCapability` (the `status` patch activates) · `DeleteCapability` · `ListCapabilities(q)` · `RecordCapabilityUsage` |
...
The engine's manual cards are **assembled in code** at Open by `internal.BuiltinCards()`, **nine cards** (English): `memhop-guide` (the loop split — Search/Update/Dream and trajectory recording run host-side and must never be manual LLM calls — plus an index of the other eight) and eight LLM-callable manuals (cycle, knowledge, scene, archive, profile, capability, trajectory, plan), **covering all 32 session methods**. A card is one **name plus its function entries** — each entry (`ResourceRef`) self-describes how it launches (`type: mcp|skill|api|composite` + `ref`/`config`), what it does (`desc`), and how to call it (`input` JSON Schema / `output`); there is no card-level type. Action chains are a `composite` entry whose `config` carries `{"steps":[{"tool":"...","args":{...}}]}`. Manual cards document the Go API (`type: "api"`, `ref: "api:MethodName"`) — the host calls the methods directly, no MCP layer involved. **Resources are tool declarations**: `name/desc/input/output` mirror the host tool spec (e.g. meowire `ToolSpec`) field-for-field, so a host projects them with a pure field copy and zero format conversion. **Tiered injection**: the manuals are **listed but never stored** (nothing persists to the `.meh` file, NOT attached to `Search` responses) — `ListCapabilities` serves them through the same filters as stored records plus a `Package` filter, a write to a manual's id reports `ErrNotFound`, and dedup is by ID with stored wins (a stored same-name card shadows the manual, deleting it restores the manual; Crystallize fold-back refuses to shadow one); inject only a one-line-per-card index (`id + name + summary + trigger`) plus the guide, and fetch full parameter schemas on demand via `ListCapabilities(CapabilityListQuery{IDs: []string{id}})`. **L5 is a file-wide pool like L3**: capability records live in a reserved shared domain every agent sees, and a `<meh-dir>/plug/` directory of packages (`plug/<pkg>/capability.json`) is auto-imported at Open — broken packages are logged and skipped.
...
|---------|------|-----------|--------------|
| v1.6.1 | 2026-09-06 | Surface re-converged (34→32); builtin manuals assembled in code (6→9 cards, covering every session method) | 1. **`ActivateCapability` deleted**: the repo layer's activate was read-card→set-active→write-back, exactly what `UpdateCapability(id, CapabilityPatch{Status: &active})` already does — one capability, one implementation; activation folds into `UpdateCapability` (and passes the same card validation, so a bare fixture without summary/resources is refused)<br>2. **`PlanReplace` deleted**: `SyncPlanTree(planID, nil)` wipes the tree (every node and bound event removed, planID kept, event Seq restarts at 1); seeding the next task is a single-node sync (an empty status lands pending); the `0000000000000000` rejection is unchanged (ParsePlanID runs before the nil check)<br>3. **MCP 31 → 30 tools**: `memhop_capability_activate` folds into `memhop_capability_update`'s `status` param<br>4. **Builtin manuals assembled in code**: the embedded `capabilities/` package is deleted (`internal.Open` drops its `fs.FS` parameter) and `internal.BuiltinCards()` builds nine cards (guide + cycle/profile/scene/knowledge/archive/capability/trajectory/plan), **covering all 32 session methods** — the core loop (Search/Update/Dream), the three trajectory methods and the three plan methods that had no manual card are now documented; cards keep stable name-derived ids, active, `Origin=builtin`, and are **listed but never stored** — a write to a manual's id reports `ErrNotFound` (the four scattered read-only guards are deleted), a stored same-name card shadows the manual (stored wins; deleting it restores the manual), and Crystallize fold-back still refuses to shadow one (`findBuiltinCapability` is the single remaining builtin check); the `plug/` auto-injection is unchanged<br>5. No format change (stays `0x000B`), no migration; after the tag, meowagent adapts in its own round (its two call sites: `ActivateCapability`/`PlanReplace`) |
