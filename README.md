...
<p align="center">
  <strong>Current: v1.7.0 (unreleased) · Latest stable tag: v1.6.0</strong>
</p>
...
- **V2 Storage** — `.meh` format (`FormatVersion=0x000B`) with A/B dual headers, per-record CRC32 + torn-write truncation recovery, mmap zero-copy, snapshot/checkpoint. Record frames carry an 8-byte `agent_id` (26-byte header) and the engine indexes every record by `(agent, idHash)` domain. 0x000A moved the L3 knowledge graph and 0x000B the L5 capability cards into reserved shared domains. **Not compatible with `0x000A` (or older) `.meh` data files** — they are rejected at Open with no migration path
...
|---------|------|-----------|--------------|
| Unreleased (v1.7.0) | 2026-09-06 | L5 unified card + file-wide capability pool + `plug/` auto-injection | 1. **One card form**: a card is name + N function entries, each entry self-describing launch (`type: mcp|skill|api|composite` + `ref`/`config`), purpose (`desc`) and usage (`input` JSON Schema/`output`); card-level `Type`/`Workflow` deleted — action chains live in a `composite` entry's `config` as `{"steps":[{"tool":...}]}`, keyed to the host's executable parser<br>2. **`memhop-capability/v4` package documents**: `{format, name, capabilities:[1..N]}` — one file = one plugin package; the package name stamps each card's `Package` field; card names are globally unique (id = hash of name, collisions rejected)<br>3. **L5 is a file-wide shared pool** like L3 (records moved into the reserved shared domain; `SharedL3AgentID` renamed `SharedPoolAgentID`, value unchanged); format 0x000A → 0x000B, older files rejected at Open, no migration<br>4. **`<meh-dir>/plug/` auto-injection**: every subfolder holding `capability.json` is imported into the pool at Open; broken packages are logged (Warn, path + reason) and skipped; byte-identical re-injection writes nothing<br>5. `ImportCapability(path)` imports a whole package and returns a per-card disposition (`created/updated/errors`); `CapabilityListQuery` gains a `Package` filter; MCP tool set unchanged (31); built-in cards rewritten as v4 single-card packages |
