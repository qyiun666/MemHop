...
// injects the embedded memhop-capability/v4 toolbox (root capabilities/
...
// toolbox (a v4 package document per file) into read-only in-memory
// capabilities with stable name-derived IDs. The files are validated by
// unit tests, so a parse failure here means a corrupted build and aborts
// Open.
func loadBuiltinCapabilities(builtins fs.FS) ([]core.Capability, error) {
	...
	for _, entry := range entries {
		...
		caps, err := capability.BuildPackage(data, "builtin:"+entry.Name())
		if err != nil {
			return nil, err
		}
