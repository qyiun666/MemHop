...
- `index/`：索引——L2Meta（场景读回的唯一话题缓存，`rebuild.go` 全量重建；它只有
  **按场景**这一种读形——`TopicsByScene` 在一把读锁下把行交出去，`RetargetScene` 一次搬完
  一个场景，全域迭代器与「先列 id 再逐个查」的那次查找随它们最后一个读者一起删了。两张表
...
- `BackfillL1Emotions` 是**先比过再写**的那一个：新值与记录上的两个分量都相同就不落这条记录，已有非零情感的节点也一律不动。第一条不是省一次写，而是**别让那个节点长生**——节点靠 `UpdatedAt` 衰减才会消失，而重写一条记录必然回戳它，于是一趟趟值相同的回灌等于每巩固一次就给这个节点续一次命，它从此不可回收。它不回传写了几个：全仓没有读者，而「几个」也说不出哪一个没落上；读写两侧失败都带着那个节点的 id 出来。
- `core` 的 Open 扫描把两种损坏当**两件事**：帧的字节数装不进文件（撕裂尾）才是截断点，从那里砍掉，后面本就不该有东西；帧完整而 CRC 不符是这一条记录的内容烂了、它的字节全在，就照它自己声明的长度跨过去，其后的记录照常进索引，被跨过的条数与第一个的位置落一条 WARN。拿 CRC 不符当截断点，就是让一条坏记录带走它后面的整段日志，而下一次 checkpoint 会把这件事变成永久的——那是把「少一条」放大成「少一半」的机制，不是恢复。跨过去的前提是 `frameSpanOf` 读得到那个长度：`RecordData` 只在确认头部与声明长度都在文件内之后才报 CRC，所以拿到 CRC 的偏移一定量得出这一帧占多宽。
