...
func BuildL1Hyperedges(engine *core.StorageEngine, agentID uint64, minSimilarity float32) (int, error) {
	...
	for _, nodes := range inverted {
		for i, a := range nodes {
			for _, b := range nodes[i+1:] {
				...
				if written, err := upsertSceneEdge(engine, agentID, lo, hi, float32(inter)/float32(union), now); err != nil {
					return changed, err
				} else if written {
					changed++
				}
...
func RebuildL1FromL2(engine *core.StorageEngine, agentID uint64, l2Meta *index.L2MetaIndex, cfg *DecayParams) ([]string, error) {
	var updated []string
...
func DecayL1Network(engine *core.StorageEngine, agentID uint64, l2Meta *index.L2MetaIndex, cfg *DecayParams) (*L1DecayReport, error) {
...
func decayNodes(engine *core.StorageEngine, agentID uint64, l2Meta *index.L2MetaIndex, cfg *DecayParams, nowMs int64, report *L1DecayReport) (map[uint64]bool, map[uint64]map[uint64]bool, error) {
...
func decayOneNode(engine *core.StorageEngine, agentID uint64, cfg *DecayParams, node *core.SceneNode, nowMs int64, report *L1DecayReport, removedNodeIDs map[uint64]bool, clearedEdges map[uint64]map[uint64]bool) error {
...
func propagateClearedEdges(engine *core.StorageEngine, agentID uint64, cfg *DecayParams, clearedEdges map[uint64]map[uint64]bool, report *L1DecayReport) error {
...
func decayRemainingEdges(engine *core.StorageEngine, agentID uint64, cfg *DecayParams, removedNodeIDs map[uint64]bool, nowMs int64, report *L1DecayReport) error {
...
func decayOneEdge(engine *core.StorageEngine, agentID uint64, cfg *DecayParams, edge *core.SceneEdge, idHash uint64, removedNodeIDs map[uint64]bool, nowMs int64, report *L1DecayReport) error {
