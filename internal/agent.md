...
4. **空闲回收**：无后台定时器；`contextFor` 顺带清扫超
   `Defaults.AgentIdleTTLMs` 未访问的域（默认域与忙碌域豁免），回收时
   快照 sparse blob 进缓存，数据仍在文件，下次访问透明重建。
5. **DeleteAgent 屏障**：先 `destroyContext`（取消 dreamCtx），再取
   `ac.mu` 屏障等待在飞操作退出，最后引擎域删除 + 租户映射清理。
