...
type MemHopDefaults struct {
	...
	// compresses; below it a scene keeps raw detail. Unlike the two knobs beside it, 0
	// does not disable anything: it is also the target the model is told to compress
	// toward, so 0 asks it to merge as far as its own rules allow.
	...
	// nodes) outlive it before a Dream sweeps them. 0 or less means the library
	// default. There is no "keep everything" spelling: retention is what bounds the
	// file, so a host needing longer-lived originals raises the window.
