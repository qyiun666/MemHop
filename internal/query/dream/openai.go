...
func (p *OpenAIProvider) Chat(
	...
	if err != nil {
		return "", mherrors.NewError(mherrors.ErrLLM, "marshal chat request", err)
	}
	req, err := http.NewRequest("POST", p.apiURL, bytes.NewReader(bodyBytes))
	if err != nil {
		return "", mherrors.NewError(mherrors.ErrLLM, "create chat request", err)
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+p.apiKey)
	resp, err := p.httpClient.Do(req)
	if err != nil {
		return "", mherrors.NewError(mherrors.ErrLLM, "chat api call failed", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		respBody, _ := io.ReadAll(resp.Body)
		return "", mherrors.NewError(mherrors.ErrLLM,
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return "", mherrors.NewError(mherrors.ErrSerialization, "parse chat response", err)
	}
	if len(result.Choices) == 0 {
		return "", mherrors.NewError(mherrors.ErrSerialization, "no content in chat response")
	}
...
func (p *OpenAIProvider) callAPI(system, user string) (string, error) {
	...
	bodyBytes, err := json.Marshal(body)
	if err != nil {
		return "", mherrors.NewError(mherrors.ErrLLM, "marshal request", err)
	}

	req, err := http.NewRequest("POST", p.apiURL, bytes.NewReader(bodyBytes))
	if err != nil {
		return "", mherrors.NewError(mherrors.ErrLLM, "create request", err)
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+p.apiKey)
	...
	resp, err := p.httpClient.Do(req)
	if err != nil {
		return "", mherrors.NewError(mherrors.ErrLLM, "api call failed", err)
	}
	defer resp.Body.Close()
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return "", mherrors.NewError(mherrors.ErrSerialization, "parse response", err)
	}
	if len(result.Choices) == 0 {
		return "", mherrors.NewError(mherrors.ErrSerialization, "no content in response")
	}
	return result.Choices[0].Message.Content, nil
}
