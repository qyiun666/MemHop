...
// DecayParamsFromConfig converts config.DecayConfig to DecayParams.
func DecayParamsFromConfig(cfg *config.DecayConfig) *DecayParams {
	if cfg == nil {
		return &DecayParams{
			LambdaNode:             0.01,
			LambdaEdge:             0.02,
...
func writeSceneNodeRecord(engine *storage.StorageEngine, id uint64, node *model.SceneNode) error {
	return record.WriteSceneNode(engine, id, node)
}
