...
func TestAverageNodeCentroidContentAddressed(t *testing.T) {
	...
	writeVecNode := func(nodeID, vecID uint64, vec []uint16) {
		...
		node := model.SceneNode{IDHash: nodeID, VectorPageRef: vecID, CreatedAt: 1, UpdatedAt: 1, EdgeIDs: []uint64{}}
		data, err := json.Marshal(node)
		if err != nil {
			t.Fatalf("marshal node: %v", err)
		}
...
func TestBackfillContextIDTypeGuard(t *testing.T) {
	...
	// Positive control: a real L1 node still gets its SceneID backfilled.
	l1NodeID := uint64(2501)
	node := model.SceneNode{IDHash: l1NodeID, CreatedAt: 1, UpdatedAt: 1, EdgeIDs: []uint64{}}
	data, err := json.Marshal(node)
	if err != nil {
		t.Fatalf("marshal node: %v", err)
	}
	...
	var got model.SceneNode
	if err := json.Unmarshal(nodeData, &got); err != nil {
		t.Fatalf("unmarshal l1 node: %v", err)
	}
	if got.SceneID != 8888 {
		t.Errorf("expected backfilled SceneID 8888, got %d", got.SceneID)
	}
}
