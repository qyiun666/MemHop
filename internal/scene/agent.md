...
- 锚点校验（`Create`/`ResolveForRead` 里的 `ReadGraphSlot`）读的是文件级公共 L3 域（`core.SharedPoolAgentID`），不是调用方自己的域：自 0x000A 起图记录全部住在公共池。`DetachGraph` 本身仍按调用方 agentID 清各自域里的场景锚点。
