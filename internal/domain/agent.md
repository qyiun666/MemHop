...
- 同一个域还管着一个不出表面的 `lastUsedStamp`：`NextUsedStamp(now)` 让该域发出的 `last_used_at`
  严格递增——字段单位是毫秒，同一毫秒里连开两轮也必须分得出先后，否则恢复时退回按场景 id 平手，
  等于让哈希决定宿主上次停在哪个会话。往前走照收，往后走不跌破本进程已发过的值；不跨重启扫描。
...
- `HasSeq` 只回答「树上有没有这一步」，拒不拒写由问它的人决定，缓存不因此知道任何
  别的东西；`Subtree` 给调用方当过滤集合，`NextSeq` 给调用方当发号器——它收一个调用方递来的下限：
  本包的树上没有那一步，事件却可以还指着它，而这份缓存不知道该往别处看一眼。
...
- `Plans` 的一个键存在当且仅当该键下还有节点；`Subtree` 对未知的根只返回它自己。
  序号没有前缀形状可匹配，树的形状只有这里知道——「这一轮的树上没人」因此不等于「可以从 1 重新发
  号」，后一个问题还要问事件轨。
- 三份缓存都从**读得回来的那部分记录**重建：一条读不回的记录在镜像里缺席，与它被保留
  窗裁掉是同一个形状。`L2Meta` 的这份缺席只影响读，留得住；`Plans` 与 `L4` 的两份还会
  决定写向哪里——序号与槽位都从幸存记录数出来（计划序号那一处数两样：幸存步骤，加上幸存事件
  点名过的序号），而记录 id 由它们派生，缺席的那条仍占着
