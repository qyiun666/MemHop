...
- 边身份是「排序后的成员 + kind」（`repo.EdgeKeyL3`），不是节点对哈希：同一对节点可以
  并存多种关系，重复导入按这个键去重。已知边的键在**建批时从盘上取回**：地址由集合派生，认不出「同一个事实」的重报会一路走到 `CreateEdgeL3` 被「这个地址已被占用」拒掉，于是宿主为一件它只是换个说法讲过的事读到一次 id 冲突。
- 一条关系 = 一条 `{source} ∪ Titles` 超边，元数不限。成员集非法（空/自指/重复/不在
  ...
  两处各写一套就会一处收紧一处放宽。
- 子图交回的必须**自足**：`AllNodesVisited` 把成员跨出深度界的边整条留下不报。一份点了某条边、
  却没把它的成员一并列进 `Nodes` 的结果，宿主只剩两条路——再跑一趟，或者把一条说不清另一端的关系
  交给模型。
