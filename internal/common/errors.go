...
const (
	ErrConfig       Code = 1001 // configuration error: missing or invalid parameters (LLM endpoint validation, open path and seeding)
	...
	ErrInvalidMagic    Code = 5003 // invalid magic bytes: .meh file header invalid (header.go)
	ErrCRCMismatch     Code = 5004 // crc32 mismatch: header/snapshot/record corruption (header/snapshot/record)
	ErrCorruption      Code = 5005 // data corruption: inconsistent file/snapshot/record structure (engine/reclaim/snapshot/record/compact)
	ErrSerialization   Code = 5006 // serialization failure: marshal and index snapshot encoding errors (record layer, db.go snapshot)
	ErrDeserialization Code = 5007 // deserialization failure: unmarshal and hypergraph/record parse errors (record layer, graph reads)
