...
## Benchmark 实测数据 (2026-05-28 最终)

### 合成 BGE-M3 测试 (100 docs, 20 queries, 10 类别)

| 模式 | NDCG@10 | R@1 | R@10 | 延迟 |
|------|---------|-----|------|------|
| **Retrieval** (HNSW+RRF) | 0.384 | 0.01 | 0.54 | **440µs** |
| Associative (Hopfield+Graph) | 0.384 | 0.01 | 0.54 | 438µs |
| Zero-vector (随机基线) | 0.144 | 0.01 | 0.17 | — |

### 延迟对比 (Latency Bench)

| 规模 | v0.8.0 P50 | v0.9.0 P50 | v0.8.0 P99 | v0.9.0 P99 |
|------|-----------|-----------|-----------|-----------|
| 1K | 5.9ms | 6.7ms | 13.6ms | 12.3ms |
| 5K | 46ms | 35.3ms | 89.9ms | 81.0ms |
| 10K | 104ms | 82.4ms | 541ms | **164.6ms** (-70%) |

### 分析

- **延迟 ✅**：HNSW 检索 440µs，P99 从 541ms → 165ms (-70%)
- **NDCG ⚠️**：合成测试 NDCG 0.38，Retrieval 和 Associative 模式一致 → RRF 融合在合成数据上没拉开差距
- **瓶颈**：剩余延迟在 ONNX 编码 (~30ms) + LMDB I/O，不在 HNSW
- **待验证**：完整 T2Retrieval 数据 + Cross-Encoder 精排后 NDCG 是否能到 0.9+
- **环境限制**：HuggingFace 不可达，无法下载 Reranker 模型和真实 benchmark 数据
